...
```powershell
node src/cli.js decode "D:\Replays\example-16.19.820.7193.rofl" `
  --events hero_minions_killed_snapshot,hero_experience_snapshot,hero_gold_earned_snapshot,hero_gold_spent_snapshot `
  --out-dir "work\16-19-hero-stats-snapshots"
```

输出包含每名英雄的候选快照值与原始包引用，并在逐能力结果中列出最后快照到回放尾部的差额。
它不推导两次 keyframe 之间的补刀、经验或金币变动时间，也不发布为确认事件。
