...
| `16.19.821.7343 --events target_hero_roster_key_pair --runtime-image PATH` | 将 `0x0265` 原生回调的非零完整 u32 与十个 `0x0089` HeroStats 阵容键做相等匹配，并保留两份原始包引用和 ROFL 元数据中的英雄、队伍、位置标签 | 写入 `target_hero_roster_key_pair_candidates`，并保存两项来源事件流；零键只计数，阵容外非零键使整场配对不可用。元数据标签本身为直接事实，键与标签的关联仍为候选；实时查找、逐包行动者、解析后的目标及效果为 `UNKNOWN` |
