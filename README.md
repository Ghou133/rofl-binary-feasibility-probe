...
| `16.19.821.7343 --events shielding_params_roster_key_pair --runtime-image PATH --event-jsonl-only` | 将原生解码的 ShieldingParams 子字段 `+0x08`、`+0x0c` 分别与同场十个 HeroStats 阵容键做完整 u32 相等匹配，保留两包及命中阵容的原始引用；11 场中 `+0x08` 命中 2,778/2,778，`+0x0c` 命中 2,770/2,778 | 写入 `shielding_params_roster_key_pair_candidates` 及两项来源事件流；8 条阵容外 `+0x0c` 保留为明确未命中且参与者字段为 `null`。两个字段的施加者、接收者、目标等角色及实际护盾效果仍为 `UNKNOWN` |
