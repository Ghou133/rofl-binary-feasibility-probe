...
| `16.19.820.7193 --events hero_inventory_broadcast` | 使用精确运行时镜像解码 HN `0x03ef` Broadcast 包中的候选槽位与物品键记录 | 仅写入 `hero_inventory_broadcast_candidates`；保留原始参数，不确认玩家归属或买卖、交换、库存变化 |
...
每条记录保留原始包来源；这三项候选不要求回放尾部 `statsJson`，也不推导库存状态或交易事件。Broadcast 记录不含已确认的玩家归属。
