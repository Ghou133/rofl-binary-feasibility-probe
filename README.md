...
| `16.19.821.7343 --events hero_roster_metadata_bridge` | 显式将十个 `0x0089` HeroStats 原始阵容键与 ROFL 结算元数据中的英雄、队伍、位置标签做候选配对；11 份 KR 回放的 110 个键都有唯一的死亡、来源击杀、助攻计数组合匹配 | 仅写入 `hero_roster_metadata_bridge_candidates`，状态为 `CANDIDATE`；上游计数本身已受结算尾部校验，这不是独立身份确认。标签是元数据原值，键与标签的关联仍为候选，逐包行动者为 `UNKNOWN`；新事件行不含 PUUID、Riot ID、玩家名或 metadata_player_id，但现有清单文件仍保留原有字段。保存查询核对 manifest 哈希与十人清单；`--verify-source` 才核对原始 ROFL 的 `statsJson` 和关键帧引用 |
...
| `16.19.821.7343 --events shielding_params_roster_key_pair --runtime-image PATH --event-jsonl-only` | 将原生解码的 ShieldingParams 子字段 `+0x08`、`+0x0c` 分别与同场十个 HeroStats 阵容键做完整 u32 相等匹配，保留两包及命中阵容的原始引用；11 场中 `+0x08` 命中 2,778/2,778，`+0x0c` 命中 2,770/2,778 | 写入 `shielding_params_roster_key_pair_candidates` 及两项来源事件流；可用 `query-events --event shielding_params_roster_key_pair_candidates --opaque-u32 VALUE` 筛选两个匿名字段，或加 `--verify-source --runtime-image PATH` 从原始回放与固定镜像重新解码全部来源。`--participant` 被拒绝。8 条阵容外 `+0x0c` 保留为明确未命中且参与者字段为 `null`。两个字段的施加者、接收者、目标等角色及实际护盾效果仍为 `UNKNOWN` |
...
| `16.19.821.7343 --events notify_contextual_situation_packet --runtime-image PATH` | 精确 821 镜像原生解码游戏流包路由 `0x0113` 的逐包 UTF-8 字符串；11 份 KR 回放共 37,229 包，观察到七种字符串；保留原始包引用 | 仅写入 `notify_contextual_situation_packet_candidates`，状态为 `CANDIDATE`；这是包路由 `0x0113`，与 OnEvent `0x040a` 的子事件 `0x0113` 不同。字符串名称不证明动作、行动者、队伍或实际效果；需要精确镜像与原生完整消费 |
| `16.19.821.7343 --events force_create_missile_packet --runtime-image PATH` | 精确 821 镜像将游戏流 `0x0087` 注册为 `PKT_S2C_ForceCreateMissile_s`；11 份 KR 回放的 206,957 包均原生完整消费，在合成接收对象下从包对象 `+0x10` 输出逐包回调比较键和原始引用 | 仅写入 `force_create_missile_packet_candidates`，状态为 `CANDIDATE`；可用 `query-events --event force_create_missile_packet_candidates --opaque-u32 VALUE` 查询；真实接收者、导弹身份、归属、目标、创建与效果仍为 `UNKNOWN` |
| `16.19.821.7343 --events change_missile_target_packet --runtime-image PATH` | 精确 821 镜像将游戏流 `0x040c` 注册为 `PKT_S2C_ChangeMissileTarget_s`；11 份 KR 回放中 7 份含此路由，共 13,125 包均原生完整消费；在合成接收对象下从包对象 `+0x1c` 输出逐包回调比较键和原始引用；显式 `--change-missile-target-v2` 另输出原生对象 `+0x10/+0x14/+0x18` 的匿名 f32 三元组（12,869 个长包的包内字节，256 个短包的原生默认零值） | V1 仍为默认，仅写入 `change_missile_target_packet_candidates`，状态为 `CANDIDATE`；V2 单独绑定原生输出摘要，可用 `query-events --event change_missile_target_packet_candidates --opaque-u32 VALUE --verify-source` 查询；其余 4 份无此路由的回放为 `PROFILE_UNAVAILABLE`；真实接收者、导弹身份、归属、目标位置、目标变化与效果仍为 `UNKNOWN` |
| `16.19.821.7343 --events set_dimension_missile_packet --runtime-image PATH` | 精确 821 镜像将游戏流 `0x008a` 注册为 `PKT_SetDimensionMissile_s`；11 份 KR 回放的 218,764 包均原生完整消费，在接收方法调用前输出包对象 `+0x14` 的受保护原始字节及回调变换后的匿名 u8 | 仅写入 `set_dimension_missile_packet_candidates`，状态为 `CANDIDATE`；真实接收者、导弹身份、归属、目标、维度变化与游戏效果均为 `UNKNOWN` |
| `16.19.821.7343 --events anonymous_029c_packet --runtime-image PATH --event-jsonl-only` | 精确 821 镜像原生完整消费 11 份 KR 回放中游戏流 `0x029c` 的 457,095 包（13 种观察形状），核对原始参数与包对象身份，并输出对象 `+0x14` 的匿名 u32 或 `0xffffffff` 哨兵及原始引用 | 仅写入 `anonymous_029c_packet_candidates`，状态为 `CANDIDATE`；每份回放最多 50,000 包；可用 `query-events --event anonymous_029c_packet_candidates --opaque-u32 VALUE --verify-source` 查询；包类名、行动者、目标、对象角色、接收状态和效果均为 `UNKNOWN` |
| `16.19.821.7343 --events anonymous_029c_roster_key_pair --runtime-image PATH --event-jsonl-only` | 只把原生完整消费后的匿名 `0x029c` **包头**完整 u32 与同场十个 HeroStats 阵容键配对；11 份回放共匹配 25,391/457,095 行，每场十键均出现；另有 414,057 个零包头与 17,647 个阵容外非零包头明确排除，其中 2,346 个为阵容键 `+0x100` 别名 | 写入 `anonymous_029c_roster_key_pair_candidates`，保留两份原始包引用及直接来自 Replay 元数据的英雄、队伍、位置标签；可用 `query-events --event anonymous_029c_roster_key_pair_candidates --opaque-u32 VALUE --verify-source` 核验，`--participant` 被拒绝。包内原生 `+0x14` 匿名 u32 独立保留，0 行与阵容键相等，不能当作参与者键；键与元数据标签的关联仍为候选，包类、行动者、目标、对象角色和效果均为 `UNKNOWN` |
| `16.19.821.7343 --events target_hero_packet --runtime-image PATH` | 精确 821 镜像将游戏流 `0x0265` 注册为 `PKT_AI_TargetHeroS2C_s`；11 份 KR 回放的 62,219 包均原生完整消费并在接收端调用前输出逐包回调 u32 与原始引用 | 仅写入 `target_hero_packet_candidates`，状态为 `CANDIDATE`；可用 `query-events --event target_hero_packet_candidates --opaque-u32 0` 查询；对象查找结果、来源行动者、目标状态和实际效果仍为 `UNKNOWN` |
| `16.19.821.7343 --events target_hero_roster_key_pair --runtime-image PATH` | 将 `0x0265` 原生回调的非零完整 u32 与十个 `0x0089` HeroStats 阵容键做相等匹配，并保留两份原始包引用和 ROFL 元数据中的英雄、队伍、位置标签 | 写入 `target_hero_roster_key_pair_candidates`，并保存两项来源事件流；可用 `query-events --event target_hero_roster_key_pair_candidates --opaque-u32 VALUE --verify-source` 核验查询，`--participant` 不适用于匿名查找键。零键只计数，阵容外非零键使整场配对不可用。元数据标签本身为直接事实，键与标签的关联仍为候选；实时查找、逐包行动者、解析后的目标及效果为 `UNKNOWN` |
...
| `16.19.821.7343 --events set_spell_level_roster_key_pair --runtime-image PATH` | 将 `0x025d` 包头完整 u32 与本场十个 `0x0089` HeroStats 阵容键做候选相等配对；配对自动运行原生 V2 包解码和阵容元数据桥 | 只把匹配行写入 `set_spell_level_roster_key_pair_candidates`；阵容外包保留在完整 `set_spell_level_packet_candidates` 来源流及排除计数中。可用 `query-events --event set_spell_level_roster_key_pair_candidates --opaque-u32 VALUE` 筛选匿名完整包头键，加 `--verify-source --runtime-image PATH` 从原始回放与固定镜像重新核验完整 V2、十人阵容及配对来源；`--participant` 被拒绝。英雄、队伍、位置为元数据标签；包行动者、归属、法术身份、实际等级变化和效果仍为 `UNKNOWN` |
...
V8 可用 `--cast-packet-v8` 显式生成，保留 V7 字段。保存结果用
