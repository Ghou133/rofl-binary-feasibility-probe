...
| `16.19.821.7343 --events change_missile_target_packet --runtime-image PATH` | 精确 821 镜像将游戏流 `0x040c` 注册为 `PKT_S2C_ChangeMissileTarget_s`；11 份 KR 回放中 7 份含此路由，共 13,125 包均原生完整消费；在合成接收对象下从包对象 `+0x1c` 输出逐包回调比较键和原始引用 | 仅写入 `change_missile_target_packet_candidates`，状态为 `CANDIDATE`；可用 `query-events --event change_missile_target_packet_candidates --opaque-u32 VALUE --verify-source` 查询；其余 4 份无此路由的回放为 `PROFILE_UNAVAILABLE`；真实接收者、导弹身份、归属、目标、目标变化与效果仍为 `UNKNOWN` |
