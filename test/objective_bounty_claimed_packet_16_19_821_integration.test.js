...
function nativeResult(request, childIds = [0x0113, 0x0101]) {
  return {
    ...
    results: request.packets.map((row, inputIndex) => {
      ...
      return {
        status: childId === 0x0113 ? 'DECODED' : 'EXCLUDED_CHILD',
        ...
        event_id: childId,
        raw_event_id_hex: childId === 0x0113 ? '0x09e5' : '0x0909',
        event_blob_length: 8,
        ...
        blob_u32_0x04: 0x40000088 + inputIndex,
      };
    }),
  };
}

test('821 objective bounty child registry and shared scan keep the exact parent shape', () => {
  const input = replay();
  assert.equal(resolveBuildProfile(input).status, 'SUPPORTED');
  assert.equal(resolveCapability(BUILD, 'objective_bounty_claimed_packet').status, 'CANDIDATE');
  const queried = capabilityQuery(input).capabilities.find((row) =>
    row.capability === 'objective_bounty_claimed_packet');
  assert.equal(queried.runtime_image_requirement, 'EXACT_IMAGE_REQUIRED');
  assert.equal(queried.output, 'objective_bounty_claimed_packet_candidates');
  const scan = collect821Routes(input, ['objective_bounty_claimed_packet']);
  assert.deepEqual(rowsFor821Capability(input, scan, 'objective_bounty_claimed_packet')
    .rows.map((row) => row.block.payload_length), [17, 17]);
});

test('821 objective bounty child API and CLI report target count separately from controls', (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-objective-entry-'));
  ...
  const missing = decodeSemanticReplay(input, {
    capabilities: ['objective_bounty_claimed_packet'],
  });
  assert.equal(missing.capability_results.objective_bounty_claimed_packet.status, 'MISSING_INPUT');
  assert.equal(missing.events, null);

  const native = t.mock.method(childProcess, 'spawnSync', (_python, args, options) => {
    assert.match(args[1], /decode_objective_bounty_claimed_packet_16_19_821\.py$/);
  ...
  const decoded = decodeSemanticReplay(input, {
    capabilities: ['objective_bounty_claimed_packet'], runtimeImagePath: imagePath,
  });
  assert.equal(decoded.status, 'EXPERIMENTAL_CANDIDATE');
  assert.equal(decoded.decoded_packet_count, 1);
  const result = decoded.capability_results.objective_bounty_claimed_packet;
  ...
  assert.equal(result.same_length_control_refs.length, 1);
  assert.deepEqual(Object.keys(decoded.events), ['objective_bounty_claimed_packet_candidates']);
  assert.equal(decoded.events.objective_bounty_claimed_packet_candidates.length, 1);
  assert.equal(decoded.events.objective_bounty_claimed_packet_candidates[0].blob_u32_0x04,
    0x40000088);
  assert.equal(decoded.events.objective_bounty_claimed_packet_candidates[0].event_blob_hex,
    'd501000088000040');

  const parsed = parseOne(filePath, {
    semantic: true, events: ['objective_bounty_claimed_packet'],
    runtimeImage: imagePath, strict: true, timelineLimit: 0,
  });
  assert.equal(parsed.ok, true);
  assert.equal(parsed.analysis.decoded_packet_count, 1);
  assert.equal(parsed.analysis.semantic.capability_results.objective_bounty_claimed_packet.status,
    'CANDIDATE');
  assert.deepEqual(Object.keys(parsed.analysis.events),
    ['objective_bounty_claimed_packet_candidates']);
  assert.equal(parsed.analysis.events.objective_bounty_claimed_packet_candidates[0].event_name,
    'OnObjectiveBountyClaimed');
  assert.equal(native.mock.callCount(), 2);
});

test('821 objective bounty child foreign-only Replay stays unavailable in selected API', (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-objective-control-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const imagePath = path.join(directory, 'image.bin');
  fs.writeFileSync(imagePath, Buffer.from([1, 2, 3]));
  ...
  assert.equal(decoded.capability_results.objective_bounty_claimed_packet.status,
    'PROFILE_UNAVAILABLE');
  assert.equal(decoded.capability_results.objective_bounty_claimed_packet.same_length_control_count,
    1);
  assert.equal(decoded.capability_results.objective_bounty_claimed_packet.same_length_control_refs.length,
    1);
