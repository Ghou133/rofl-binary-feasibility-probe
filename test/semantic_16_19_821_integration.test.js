...
test('821 build exposes only its exact candidate and tail-only preflight', () => {
  ...
  assert.equal(queried.params_heal_packet.runtime_image_requirement,
    'EXACT_IMAGE_REQUIRED');
  assert.deepEqual(queried.params_heal_packet.missing_inputs,
    ['exact_runtime_image']);
  assert.equal(queried.params_heal_packet.output,
    'params_heal_packet_candidates');
  assert.equal(queried.shielding_params_packet_pair.runtime_image_requirement,
    'EXACT_IMAGE_REQUIRED');
  assert.deepEqual(queried.shielding_params_packet_pair.missing_inputs,
    ['exact_runtime_image']);
  ...
  assert.equal(queried.champion_die_event_packet.runtime_image_requirement,
    'EXACT_IMAGE_REQUIRED');
  assert.deepEqual(queried.champion_die_event_packet.missing_inputs,
    ['exact_runtime_image']);
  assert.equal(queried.champion_die_event_packet.output,
    'champion_die_event_packet_candidates');
  assert.equal(queried.champion_kill_event_packet.runtime_image_requirement,
    'EXACT_IMAGE_REQUIRED');
  assert.deepEqual(queried.champion_kill_event_packet.missing_inputs,
    ['exact_runtime_image']);
  assert.equal(queried.champion_kill_event_packet.output,
    'champion_kill_event_packet_candidates');
  assert.equal(queried.champion_multiple_kill_event_packet.runtime_image_requirement,
    'EXACT_IMAGE_REQUIRED');
  assert.deepEqual(queried.champion_multiple_kill_event_packet.missing_inputs,
    ['exact_runtime_image']);
  assert.equal(queried.champion_multiple_kill_event_packet.output,
  ...
  assert.equal(queried.set_movement_driver_packet.runtime_image_requirement,
    'EXACT_IMAGE_REQUIRED');
  assert.deepEqual(queried.set_movement_driver_packet.missing_inputs,
    ['exact_runtime_image']);
  assert.equal(queried.set_movement_driver_packet.output,
    'set_movement_driver_packet_candidates');
