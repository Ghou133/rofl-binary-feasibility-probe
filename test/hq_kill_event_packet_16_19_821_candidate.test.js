...
const {
  HQ_KILL_EVENT_PACKET_821_PROFILE: profile,
  decodeHqKillEventPacketCandidates821: decode,
...
const FOREIGN_SUFFIXES = ['247152269548', '247152269506',
  '247152269566', '247152269586'];

function shapedPayload(suffix = '247152269518') {
...
function fakeImage(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-hq-kill-test-'));
...
test('821 OnHQKill selects native checked target and preserves four foreign refs', (t) => {
  ...
  assert.equal(profile.child_event_name, 'OnHQKill');
  ...
  assert.equal(result.events[0].event_type, 'HQ_KILL_EVENT_PACKET_CANDIDATE');
  assert.equal(result.events[0].event_id, 0x0046);
  assert.equal(result.events[0].event_name, 'OnHQKill');
  ...
  for (const field of ['hq_destroyed', 'winner', 'actor', 'target']) {
    assert.equal(field in result.events[0], false);
  }
  assert.equal(invoke.mock.callCount(), 1);
});

test('821 OnHQKill reports absent shape, missing image, wrong build, and corrupt Replay', () => {
...
test('821 OnHQKill rejects foreign stream, unknown length-116 shape, and wrong image', (t) => {
  const image = fakeImage(t);
  ...
  t.mock.method(childProcess, 'spawnSync', () => ({
    status: 1, stderr: 'OnHQKill runtime image SHA-256 mismatch: bad', stdout: '',
  }));
...
test('821 OnHQKill rejects native child, partial decode, and altered blob atomically', (t) => {
  const image = fakeImage(t);
