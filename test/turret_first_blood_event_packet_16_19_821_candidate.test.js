...
const {
  TURRET_FIRST_BLOOD_EVENT_PACKET_821_PROFILE: profile,
  decodeTurretFirstBloodEventPacketCandidates821: decode,
...
const FOREIGN_SUFFIXES = ['247152269548', '247152269506',
  '247152269566', '247152269518'];

function shapedPayload(suffix = '247152269586') {
...
function fakeImage(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-turret-first-blood-test-'));
...
test('821 OnTurretFirstBlood selects native checked target and preserves four foreign refs', (t) => {
  ...
  assert.equal(profile.child_event_name, 'OnTurretFirstBlood');
  ...
  assert.equal(result.events[0].event_type, 'TURRET_FIRST_BLOOD_EVENT_PACKET_CANDIDATE');
  assert.equal(result.events[0].event_id, 0x003d);
  assert.equal(result.events[0].event_name, 'OnTurretFirstBlood');
  ...
  for (const field of ['first_blood', 'turret_destroyed', 'actor', 'target']) {
    assert.equal(field in result.events[0], false);
  }
  assert.equal(invoke.mock.callCount(), 1);
});

test('821 OnTurretFirstBlood reports absent shape, missing image, wrong build, and corrupt Replay', () => {
...
test('821 OnTurretFirstBlood rejects foreign stream, unknown length-116 shape, and wrong image', (t) => {
  const image = fakeImage(t);
  ...
  t.mock.method(childProcess, 'spawnSync', () => ({
    status: 1, stderr: 'OnTurretFirstBlood runtime image SHA-256 mismatch: bad', stdout: '',
  }));
...
test('821 OnTurretFirstBlood rejects native child, partial decode, and altered blob atomically', (t) => {
  const image = fakeImage(t);
