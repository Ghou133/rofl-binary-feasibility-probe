...
const LOCAL_INPUTS = fs.existsSync(IMAGE) && fs.existsSync(ROWS) && fs.existsSync(SOURCE);
...
function evidenceRows() {
  ...
  const target = rows.filter((row) => row.child_event_id_hex === '0x003b');
  assert.equal(target.length, 136);
  assert.equal(new Set(target.map((row) => row.replay_name)).size, 11);
  assert.ok(target.every((row) => row.payload_length === 116
    && row.native_full_consumed && row.native_blob_length === 108
    && row.raw_event_id_hex === '0x4966'));
  return target;
}
...
test('821 native OnTurretDie helper fully decodes 136 observed child 0x003b packets', {
  ...
  for (const [index, result] of output.results.entries()) {
    ...
    assert.equal(result.event_id, 0x003b);
    assert.equal(result.raw_event_id_hex, '0x4966');
...
test('821 native OnTurretDie helper rejects all four real same-length foreign children atomically', {
  skip: !LOCAL_INPUTS ? 'requires exact-build image and ignored raw-row evidence' : false,
}, () => {
...
test('821 native OnTurretDie helper rejects malformed route, build, payload, and image', {
  skip: !LOCAL_INPUTS ? 'requires exact-build image and ignored raw-row evidence' : false,
}, () => {
  const first = packet(evidenceRows()[0]);
  const controls = [
