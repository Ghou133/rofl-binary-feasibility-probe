...
const { NPC_BUFF_UPDATE_NUM_COUNTER_PACKET_CANDIDATE_PROFILE_821: profile } =
...
function nativeResult(request) {
  return {
    ...
    results: request.packets.map((row, inputIndex) => ({
      ...
      native_packet_id: 0x0194, native_raw_param: row.raw_param,
      opaque_u8_0x10: inputIndex,
      opaque_u32_0x14: 0x400000af + inputIndex,
      opaque_u8_0x18: 1 + inputIndex,
      opaque_u32_0x1c: 5 + inputIndex,
      raw_u8_0x10_hex: '00',
...
test('821 BuffUpdateNumCounter registry and shared scan keep only packet 0x0194', () => {
  const input = replay([
    packet(0x0194, 6), packet(0x00ae, 13), packet(0x0194, 9, 0x400000af),
  ]);
  assert.equal(resolveBuildProfile(input).status, 'SUPPORTED');
  assert.equal(resolveCapability(BUILD, 'npc_buff_update_num_counter_packet').status,
    'CANDIDATE');
  const queried = capabilityQuery(input).capabilities.find((row) =>
    row.capability === 'npc_buff_update_num_counter_packet');
  assert.equal(queried.runtime_image_requirement, 'EXACT_IMAGE_REQUIRED');
  assert.equal(queried.output, 'npc_buff_update_num_counter_packet_candidates');
  const scan = collect821Routes(input, ['npc_buff_update_num_counter_packet']);
  assert.deepEqual(rowsFor821Capability(input, scan, 'npc_buff_update_num_counter_packet')
    .rows.map((row) => row.block.payload_length), [6, 9]);
});

test('821 BuffUpdateNumCounter API and CLI emit anonymous packet candidates', (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-buff-counter-entry-'));
  ...
  const missing = decodeSemanticReplay(input, {
    capabilities: ['npc_buff_update_num_counter_packet'],
  });
  assert.equal(missing.capability_results.npc_buff_update_num_counter_packet.status,
    'MISSING_INPUT');
  ...
  const native = t.mock.method(childProcess, 'spawnSync', (_python, args, options) => {
    ...
    assert.deepEqual(request.packets.map((row) =>
      [row.packet_id, row.payload_hex.length / 2]), [[0x0194, 6], [0x0194, 9]]);
    return { status: 0, stderr: '', stdout: JSON.stringify(nativeResult(request)) };
  });
  const decoded = decodeSemanticReplay(input, {
    capabilities: ['npc_buff_update_num_counter_packet'], runtimeImagePath: imagePath,
  });
  assert.equal(decoded.status, 'EXPERIMENTAL_CANDIDATE');
  assert.equal(decoded.decoded_packet_count, 2);
  assert.equal(decoded.capability_results.npc_buff_update_num_counter_packet.input_count, 2);
  assert.equal(decoded.capability_results.npc_buff_update_num_counter_packet.event_count, 2);
  assert.deepEqual(Object.keys(decoded.events),
    ['npc_buff_update_num_counter_packet_candidates']);
  assert.deepEqual(decoded.events.npc_buff_update_num_counter_packet_candidates
    .map((row) => row.opaque_u32_0x14), [0x400000af, 0x400000b0]);
  assert.equal(decoded.events.npc_buff_update_num_counter_packet_candidates[0]
    .raw_packet_ref.packet_id, 0x0194);
  for (const key of ['buff_id', 'owner', 'counter', 'target']) {
    assert.equal(key in decoded.events.npc_buff_update_num_counter_packet_candidates[0],
      false);
  }

  const parsed = parseOne(filePath, {
    semantic: true, events: ['npc_buff_update_num_counter_packet'],
    runtimeImage: imagePath, strict: true, timelineLimit: 0,
  });
  assert.equal(parsed.ok, true);
  assert.equal(parsed.analysis.semantic.capability_results
    .npc_buff_update_num_counter_packet.status, 'CANDIDATE');
