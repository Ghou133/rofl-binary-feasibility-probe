...
const ARTIFACT_ROOT = path.join(
  ROOT, 'artifacts', 'full_semantic_deep_recovery_v2', 'residual_p2_wave',
);
...
test('residual P2 scope is exact and stable', () => {
  assert.equal(EXACT_BUILD, '16.16.805.0442');
  assert.equal(RUNTIME_SHA256.length, 64);
  assert.equal(ROUTE_IDS.length, 12);
  assert.equal(new Set(ROUTE_IDS).size, 12);
  assert.equal(Object.keys(FAMILY_BY_ROUTE).length, 12);
...
test('generated report conserves the complete twelve-route inventory', () => {
  const report = readJson('residual_p2_wave_audit_16_16.json');
  ...
  assert.equal(report.conservation.target_row_count, 134679);
...
test('factory chains, deserializer fields, and bounded callback-negative surface are exact', () => {
  const report = readJson('residual_p2_static_runtime_16_16.json');
  ...
  assert.equal(report.structural_family_relationships.four_variant
    .constructors_exactly_0x40_apart, true);
  assert.equal(report.structural_family_relationships.four_variant
    .vtables_exactly_0x30_apart_reverse, true);
  assert.equal(report.structural_family_relationships.two_variant
    .constructors_exactly_0x40_apart, true);
  assert.equal(report.structural_family_relationships.two_variant.equal_object_size, true);
...
test('machine decisions exhaust route, capability, and domain local hypotheses', () => {
  const decisions = readJson('residual_p2_wave_decisions_16_16.json');
