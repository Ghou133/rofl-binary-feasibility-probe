...
const {
  OBJECTIVE_BOUNTY_CLAIMED_PACKET_821_PROFILE: profile,
  decodeObjectiveBountyClaimedEventPacketCandidates821: decode,
...
function fakeImage(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-objective_bounty_claimed-test-'));
...
function nativeResult(request, childIds = [0x0113, 0x0101, 0x0113]) {
  const rawIds = new Map([[0x0113, '0x09e5'], [0x0101, '0x0909'],
    [0x0102, '0x0929'], [0x0107, '0x09e8']]);
  return {
    status: 'PASS', runtime_image_sha256: IMAGE_SHA256,
    results: request.packets.map((row, index) => {
      const childId = childIds[index] ?? 0x0113;
      ...
      return {
        ...
        blob_u32_0x04: 0x40000088 + index,
      };
    }),
  };
}

test('821 OnObjectiveBountyClaimed excludes same-length foreign children and keeps raw refs', (t) => {
  ...
  assert.equal(profile.child_event_name, 'OnObjectiveBountyClaimed');
  assert.equal(profile.payload_length, 17);
  assert.equal(profile.event_blob_length, 8);
  ...
  assert.equal(result.same_length_control_refs[0].child_event_id, 0x0101);
  assert.equal(result.same_length_control_refs[0].raw_packet_ref.raw_param,
    0x400000af);
  assert.equal(result.runtime_image_status, 'MATCHED_USED');
  assert.deepEqual(result.events.map((row) => row.blob_u32_0x04),
    [0x40000088, 0x4000008a]);
  assert.equal(result.events[0].event_type, 'OBJECTIVE_BOUNTY_CLAIMED_PACKET_CANDIDATE');
  assert.equal(result.events[0].event_id, 0x0113);
  assert.equal(result.events[0].event_blob_hex, 'd501000088000040');
  assert.equal(result.events[0].event_blob_sha256,
    crypto.createHash('sha256').update(Buffer.from(result.events[0].event_blob_hex, 'hex'))
      .digest('hex'));
  ...
  for (const field of ['bounty_awarded', 'actor', 'team', 'structure', 'event_u32_0x04']) {
    assert.equal(field in result.events[0], false);
  }
  assert.equal(invoke.mock.callCount(), 1);
});

test('821 OnObjectiveBountyClaimed distinguishes absent shape, missing image, wrong build, and corrupt Replay', () => {
...
test('821 OnObjectiveBountyClaimed reports foreign-only native packets as unavailable', (t) => {
  const image = fakeImage(t);
  ...
  assert.equal(result.same_length_control_refs.length, 2);
  assert.equal(result.runtime_image_status, 'MATCHED_USED');
});

test('821 OnObjectiveBountyClaimed rejects foreign stream and wrong image digest', (t) => {
  const image = fakeImage(t);
  ...
  t.mock.method(childProcess, 'spawnSync', () => ({
    status: 1, stderr: 'OnObjectiveBountyClaimed runtime image SHA-256 mismatch: bad', stdout: '',
  }));
...
test('821 OnObjectiveBountyClaimed binds native ID and anonymous u32 to returned bytes atomically', (t) => {
  const image = fakeImage(t);
  ...
  t.mock.method(childProcess, 'spawnSync', (_python, _args, options) => {
    ...
    if (failure === 'field') result.results[2].blob_u32_0x04 += 1;
