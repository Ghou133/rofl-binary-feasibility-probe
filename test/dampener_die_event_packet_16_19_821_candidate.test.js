...
const {
  DAMPENER_DIE_EVENT_PACKET_821_PROFILE: profile,
  decodeDampenerDieEventPacketCandidates821: decode,
} = require('../src/decoders/rofl_16_19_821_dampener_die_event_packet_candidate');

const BUILD = '16.19.821.7343';
const IMAGE_SHA256 = '35b49575122a8b063d5db6b37373f59740aa25b4be28d0affcb12f93be0cd325';
const FOREIGN_SUFFIXES = ['247152269548', '247152269566',
  '247152269586', '247152269518'];

function shapedPayload(suffix = '247152269506') {
...
function fakeImage(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-dampener-die-test-'));
...
test('821 OnDampenerDie selects native checked target and preserves four foreign refs', (t) => {
  ...
  assert.equal(profile.child_event_name, 'OnDampenerDie');
  ...
  assert.equal(result.events[0].event_type, 'DAMPENER_DIE_EVENT_PACKET_CANDIDATE');
  assert.equal(result.events[0].event_id, 0x0035);
  assert.equal(result.events[0].event_name, 'OnDampenerDie');
  ...
  for (const field of ['dampener_destroyed', 'actor', 'target', 'effective_death']) {
    assert.equal(field in result.events[0], false);
  }
  assert.equal(invoke.mock.callCount(), 1);
});

test('821 OnDampenerDie reports absent shape, missing image, wrong build, and corrupt Replay', () => {
...
test('821 OnDampenerDie rejects foreign stream, unknown length-116 shape, and wrong image', (t) => {
  const image = fakeImage(t);
  ...
  const wrongPrefix = shapedPayload();
  wrongPrefix[1] ^= 1;
  const wrongPrefixResult = decode(replayWithChunks([{
    packets: [packet(0x040a, 0x4000008d, wrongPrefix)],
  }]), { runtimeImagePath: image });
  assert.equal(wrongPrefixResult.status, 'DECODE_FAILED');
  assert.match(wrongPrefixResult.error, /unrecognized.*fingerprint/i);
  assert.equal(wrongPrefixResult.events, null);
  const replay = replayWithChunks([{ packets: [packet()] }]);
  t.mock.method(childProcess, 'spawnSync', () => ({
    status: 1, stderr: 'OnDampenerDie runtime image SHA-256 mismatch: bad', stdout: '',
  }));
...
test('821 OnDampenerDie rejects native child, partial decode, and altered blob atomically', (t) => {
  const image = fakeImage(t);
  ...
  t.mock.method(childProcess, 'spawnSync', (_python, _args, options) => {
    ...
    if (failure === 'foreign_id') result.results[1].event_id = 0x003b;
