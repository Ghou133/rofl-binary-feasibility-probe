...
const {
  ...
  HERO_TOTAL_TIME_SPENT_DEAD_SNAPSHOT_CANDIDATE_PROFILE: profile,
  analyzeReplayWithHeroStats,
  assessHeroTotalTimeSpentDeadSnapshotTail: assessTail,
  decodeHeroStatsByte,
  decodeHeroTotalTimeSpentDeadPayload: decodePayload,
  decodeHeroTotalTimeSpentDeadSnapshotCandidates: decodeCandidates,
...
const CAPABILITY = 'hero_total_time_spent_dead_snapshot';
const TAIL_FIELD = 'TOTAL_TIME_SPENT_DEAD';
const RAW_KEY = 'total_time_spent_dead_raw_f32_candidate';
const FLOOR_KEY = 'total_time_spent_dead_floor_candidate';
...
function payloadFor(value, neighbors = 0) {
  ...
  blob.writeFloatLE(value, 0x248);
  blob.writeFloatLE(neighbors, 0x244);
  blob.writeFloatLE(neighbors, 0x24c);
...
test('exact HN candidate decodes f32 0x248 independently of neighboring fields', () => {
  ...
  assert.equal(profile.total_time_spent_dead_f32le_offset_candidate, 0x248);
...
test('selected API emits raw f32, derived floor, packet provenance and tail gaps', () => {
  ...
  assert.deepEqual(outcome.tail_gaps[1], {
    ...
    last_snapshot_raw_f32_candidate: 10.25,
    last_snapshot_floor_candidate: 10,
    final_tail: 11,
    unobserved_tail_floor_gap: 1,
    unobserved_tail_time_ms: 1000,
  });
  assert.equal(outcome.tail_gaps[4].last_snapshot_floor_candidate, 0);
  assert.equal(outcome.tail_gaps[4].final_tail, 0);
  const events = getHeroTotalTimeSpentDeadSnapshotCandidates(decoded);
  assert.equal(events.length, 20);
  const last = events.at(-1);
  assert.equal(last.event_type, 'HERO_TOTAL_TIME_SPENT_DEAD_SNAPSHOT_CANDIDATE');
  ...
  for (const key of ['death_time_ms', 'death_duration_ms', 'respawn_time_ms']) {
...
test('missing tail, overrun, decline, wrong build and incomplete roster fail closed', () => {
  ...
  assert.match(decodeCandidates(overrun).error,
    /exceeds Replay tail TOTAL_TIME_SPENT_DEAD/);
  ...
  assert.match(decodeCandidates(decline).error,
    /decreasing observed TOTAL_TIME_SPENT_DEAD/);
...
test('same-Replay precollected scan and CLI capability preflight expose only selected candidate', () => {
  const replay = fixture();
  ...
  const queryReplay = fixture();
  const entry = capabilityQuery(queryReplay).capabilities.find((row) =>
  ...
  delete queryReplay.tail.stats[0][TAIL_FIELD];
  const missing = capabilityQuery(queryReplay).capabilities.find((row) =>
    row.capability === CAPABILITY);
