...
test('exact 821 native lookup key reaches API and CLI with the same source-bound rows',
  async (t) => {
    ...
    assert.equal(result.event_count, 6888);
    assert.equal(result.matched_lookup_key_packet_count, result.event_count);
    assert.equal(result.matched_equal_packet_count, 5254);
    assert.equal(result.matched_alias_0x100_packet_count
      + result.matched_other_relation_packet_count, 1634);
    ...
    assert.ok(rows.every((row) =>
      row.event_type === 'UNIT_APPLY_DAMAGE_LOOKUP_ROSTER_KEY_CANDIDATE'
        ...
        && row.hero_raw_param === row.native_callback_lookup_key_u32_0x24_candidate
    ...
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-damage-lookup-roster-'));
...
test('missing image keeps an independent level candidate while lookup association is unavailable',
  (t) => {
