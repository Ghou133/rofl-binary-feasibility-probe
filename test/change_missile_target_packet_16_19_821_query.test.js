...
const EVENT = 'change_missile_target_packet_candidates';
const SOURCE_PATH = 'synthetic.rofl';
const REPLAY_SHA = 'a'.repeat(64);
const PACKETS = [
  ['9656981069287b66ec2cad7569', 'c9c9c9c9', 0],
  ['916fcb', '6fc9c94a', 0x400000b2],
];
...
test('saved missile comparison key query emits original rows and filters only observed key', (t) => {
  ...
  for (const [, protectedHex, key] of PACKETS) {
    assert.equal(decodeProtectedChangeMissileTargetComparisonKeyU32(protectedHex), key);
  }
  ...
  const noMatch = query(f.dir, '--opaque-u32', '1');
...
test('missile source verification checks every physical packet after the output limit', (t) => {
  ...
  const promoted = structuredClone(physical.rows);
  promoted[1].live_receiver_comparison_status = 'MATCHED';
  fs.writeFileSync(f.eventPath, `${promoted.map(JSON.stringify).join('\n')}\n`);
  const rejectedPromotion = query(f.dir, '--verify-source', '--limit', '1');
  assert.equal(errorCode(rejectedPromotion), 'INVALID_EVENT_ROW');
  assert.equal(rejectedPromotion.stdout, '');
...
test('later altered raw payload with matching row SHA fails ordered native input digest', (t) => {
  const f = fixture(t);
  const forged = structuredClone(f.rows);
  forged[1].raw_packet_ref.raw_payload_hex = '936fcb';
  forged[1].raw_packet_ref.raw_payload_sha256 = sha256(Buffer.from('936fcb', 'hex'));
  fs.writeFileSync(f.eventPath, `${forged.map(JSON.stringify).join('\n')}\n`);
  const selected = query(f.dir, '--limit', '1');
  assert.equal(errorCode(selected), 'EVENT_COUNT_MISMATCH');
  assert.equal(selected.stdout, '');
...
test('later promoted receiver or malformed comparison key is rejected without output', (t) => {
  const f = fixture(t);
  for (const mutate of [
    (item) => { item.live_receiver_comparison_status = 'MATCHED'; },
...
test('candidate metadata and exact image remain fail closed', (t) => {
  ...
  for (const change of [
    ...
    (result) => { result.input_packet_id = 0x0087; },
  ]) {
