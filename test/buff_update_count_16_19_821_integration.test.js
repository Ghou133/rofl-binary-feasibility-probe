...
const { NPC_BUFF_UPDATE_COUNT_PACKET_CANDIDATE_PROFILE_821: profile } =
...
function nativeResult(request) {
  return {
    ...
    results: request.packets.map((row, inputIndex) => ({
      ...
      native_packet_id: 0x02d9, native_raw_param: row.raw_param,
      opaque_u8_0x10: inputIndex,
      opaque_u8_0x11: 1 + inputIndex,
      opaque_u32_0x14: 0x400000af + inputIndex,
      opaque_f32_0x18: 0.5 + inputIndex,
      opaque_f32_0x1c: 1.5 + inputIndex,
      raw_u8_0x10_hex: '00',
      raw_u8_0x11_hex: '00',
      raw_u32_0x14_hex: '00000000',
      raw_f32_0x18_hex: '00000000',
      raw_f32_0x1c_hex: '00000000',
    })),
  };
}

test('821 BuffUpdateCount registry and shared scan keep only packet 0x02d9', () => {
  const input = replay([
    packet(0x02d9, 4), packet(0x0194, 6), packet(0x02d9, 15, 0x400000af),
  ]);
  assert.equal(resolveBuildProfile(input).status, 'SUPPORTED');
  assert.equal(resolveCapability(BUILD, 'npc_buff_update_count_packet').status,
    'CANDIDATE');
  const queried = capabilityQuery(input).capabilities.find((row) =>
    row.capability === 'npc_buff_update_count_packet');
  assert.equal(queried.runtime_image_requirement, 'EXACT_IMAGE_REQUIRED');
  assert.equal(queried.output, 'npc_buff_update_count_packet_candidates');
  const scan = collect821Routes(input, ['npc_buff_update_count_packet']);
  assert.deepEqual(rowsFor821Capability(input, scan, 'npc_buff_update_count_packet')
    .rows.map((row) => row.block.payload_length), [4, 15]);
});

test('821 BuffUpdateCount API and CLI emit anonymous packet candidates', (t) => {
  ...
  const missing = decodeSemanticReplay(input, {
    capabilities: ['npc_buff_update_count_packet'],
  });
  assert.equal(missing.capability_results.npc_buff_update_count_packet.status,
    'MISSING_INPUT');
  ...
  const native = t.mock.method(childProcess, 'spawnSync', (_python, args, options) => {
    ...
    assert.deepEqual(request.packets.map((row) =>
      [row.packet_id, row.payload_hex.length / 2]), [[0x02d9, 4], [0x02d9, 15]]);
    return { status: 0, stderr: '', stdout: JSON.stringify(nativeResult(request)) };
  });
  const decoded = decodeSemanticReplay(input, {
    capabilities: ['npc_buff_update_count_packet'], runtimeImagePath: imagePath,
  });
  assert.equal(decoded.status, 'EXPERIMENTAL_CANDIDATE');
  assert.equal(decoded.decoded_packet_count, 2);
  assert.equal(decoded.capability_results.npc_buff_update_count_packet.input_count, 2);
  assert.equal(decoded.capability_results.npc_buff_update_count_packet.event_count, 2);
  assert.deepEqual(Object.keys(decoded.events),
    ['npc_buff_update_count_packet_candidates']);
  assert.deepEqual(decoded.events.npc_buff_update_count_packet_candidates
    .map((row) => row.opaque_u32_0x14), [0x400000af, 0x400000b0]);
  assert.equal(decoded.events.npc_buff_update_count_packet_candidates[0]
    .raw_packet_ref.packet_id, 0x02d9);
  for (const key of ['buff_id', 'owner', 'counter', 'target']) {
    assert.equal(key in decoded.events.npc_buff_update_count_packet_candidates[0], false);
  }

  const parsed = parseOne(filePath, {
    semantic: true, events: ['npc_buff_update_count_packet'],
    runtimeImage: imagePath, strict: true, timelineLimit: 0,
  });
  assert.equal(parsed.ok, true);
  assert.equal(parsed.analysis.semantic.capability_results
    .npc_buff_update_count_packet.status, 'CANDIDATE');
