...
test('821 native OnHQKill helper fully decodes 11 observed child 0x0046 packets', {
  ...
  for (const [index, result] of output.results.entries()) {
    ...
    assert.equal(result.event_id, 0x0046);
    assert.equal(result.raw_event_id_hex, '0x4918');
    ...
    for (const field of ['actor', 'target', 'hq', 'winner',
      'effective_destruction', 'state_transition']) {
      assert.equal(Object.hasOwn(result, field), false);
    }
  }
});

test('821 native OnHQKill helper rejects all four real same-length foreign children atomically', {
  skip: !LOCAL_INPUTS ? 'requires exact-build image and ignored raw-row evidence' : false,
}, () => {
...
test('821 native OnHQKill helper rejects malformed route, build, payload, and image', {
  skip: !LOCAL_INPUTS ? 'requires exact-build image and ignored raw-row evidence' : false,
}, () => {
  const first = packet(evidenceRows()[0]);
  const controls = [
