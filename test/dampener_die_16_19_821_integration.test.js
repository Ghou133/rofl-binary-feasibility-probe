...
const TARGET = payloadWithRawChildSuffix('247152269506');
  ...
  return {
    ...
    results: request.packets.map((row, inputIndex) => {
      ...
      return {
        ...
        event_id: 0x0035, raw_event_id_hex: '0x4906',
...
test('821 OnDampenerDie registry and shared scan expose only the exact candidate route', () => {
  const input = replay([TARGET, FOREIGN, Buffer.alloc(20)]);
  assert.equal(resolveBuildProfile(input).status, 'SUPPORTED');
  assert.equal(resolveCapability(BUILD, 'dampener_die_event_packet').status, 'CANDIDATE');
  const queried = capabilityQuery(input).capabilities.find((row) =>
    row.capability === 'dampener_die_event_packet');
  assert.equal(queried.runtime_image_requirement, 'EXACT_IMAGE_REQUIRED');
  assert.equal(queried.output, 'dampener_die_event_packet_candidates');
  const scan = collect821Routes(input,
    ['dampener_die_event_packet', 'revive_ally_event_packet']);
  assert.deepEqual(rowsFor821Capability(input, scan, 'dampener_die_event_packet')
    .rows.map((row) => row.block.payload_length), [116, 116]);
  assert.deepEqual(rowsFor821Capability(input, scan, 'revive_ally_event_packet')
    .rows.map((row) => row.block.payload_length), []);
});

test('821 OnDampenerDie API and CLI emit native target and retain foreign exclusions', (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-dampener-die-entry-'));
  ...
  const missing = decodeSemanticReplay(input, {
    capabilities: ['dampener_die_event_packet'],
  });
  assert.equal(missing.capability_results.dampener_die_event_packet.status, 'MISSING_INPUT');
  assert.equal(missing.events, null);

  const native = t.mock.method(childProcess, 'spawnSync', (_python, args, options) => {
    assert.match(args[1], /decode_dampener_die_event_packet_16_19_821\.py$/);
  ...
  const decoded = decodeSemanticReplay(input, {
    capabilities: ['dampener_die_event_packet'], runtimeImagePath: imagePath,
  });
  assert.equal(decoded.status, 'EXPERIMENTAL_CANDIDATE');
  assert.equal(decoded.capability_results.dampener_die_event_packet.runtime_image_status,
    'MATCHED_USED');
  assert.equal(decoded.capability_results.dampener_die_event_packet.excluded_same_length_foreign_count, 1);
  assert.equal(decoded.events.dampener_die_event_packet_candidates.length, 1);

  const parsed = parseOne(filePath, {
    semantic: true, events: ['dampener_die_event_packet'],
    runtimeImage: imagePath, strict: true, timelineLimit: 0,
  });
  assert.equal(parsed.ok, true);
  assert.equal(parsed.analysis.semantic.capability_results.dampener_die_event_packet.status,
    'CANDIDATE');
  assert.deepEqual(Object.keys(parsed.analysis.events),
    ['dampener_die_event_packet_candidates']);
  assert.equal(parsed.analysis.events.dampener_die_event_packet_candidates[0].event_name,
    'OnDampenerDie');
  ...
  assert.equal(absent.analysis.semantic.capability_results.dampener_die_event_packet.status,
    'PROFILE_UNAVAILABLE');
  assert.equal(absent.analysis.semantic.capability_results.dampener_die_event_packet.event_count,
    null);
