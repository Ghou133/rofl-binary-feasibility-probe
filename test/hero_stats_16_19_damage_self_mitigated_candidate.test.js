...
const {
  ...
  HERO_DAMAGE_SELF_MITIGATED_SNAPSHOT_CANDIDATE_PROFILE: profile,
  analyzeReplayWithHeroStats,
  assessHeroDamageSelfMitigatedSnapshotTail: assessTail,
  decodeHeroStatsByte,
  decodeHeroDamageSelfMitigatedPayload: decodePayload,
  decodeHeroDamageSelfMitigatedSnapshotCandidates: decodeCandidates,
...
const CAPABILITY = 'hero_damage_self_mitigated_snapshot';
const TAIL_FIELD = 'TOTAL_DAMAGE_SELF_MITIGATED';
const RAW_KEY = 'damage_self_mitigated_raw_f32_candidate';
const FLOOR_KEY = 'damage_self_mitigated_floor_candidate';
...
function payloadFor(value, neighbors = 0) {
  ...
  blob.writeFloatLE(value, 0x208);
  blob.writeFloatLE(neighbors, 0x204);
  blob.writeFloatLE(neighbors, 0x20c);
...
function fixture({ version = BUILD, stream = 2, omitParticipant = null,
  malformed = false, rows = [Array(10).fill(0), Array.from({ length: 10 },
    (_, index) => (index + 1) * 3 + 0.5)] } = {}) {
...
test('exact HN candidate reads decoded f32 at 0x208 independently of its neighbors', () => {
  ...
  assert.equal(profile.damage_self_mitigated_f32le_offset_candidate, 0x208);
  assert.match(profile.evidence_scope, /two HN Replays/);
  assert.match(profile.known_limits.join(' '), /not exact-runtime field semantics/);
...
test('selected API keeps exact build, provenance, raw f32, derived floor and tail gap', () => {
  ...
  const events = getHeroDamageSelfMitigatedSnapshotCandidates(decoded);
  assert.equal(events.length, 20);
  const last = events.at(-1);
  assert.equal(last.event_type, 'HERO_DAMAGE_SELF_MITIGATED_SNAPSHOT_CANDIDATE');
  ...
  assert.equal(last.raw_packet_ref.raw_payload_sha256,
    crypto.createHash('sha256').update(payloadFor(30.5, 9000)).digest('hex'));
...
test('missing tail, overrun, decline, wrong build and incomplete roster fail closed', () => {
  ...
  assert.match(decodeCandidates(overrun).error,
    /exceeds Replay tail TOTAL_DAMAGE_SELF_MITIGATED/);
  ...
  assert.match(decodeCandidates(decline).error,
    /decreasing observed TOTAL_DAMAGE_SELF_MITIGATED/);
...
test('precollected same-Replay scan is reused without another chunk walk', () => {
  const replay = fixture();
  ...
  const reused = decodeHeroStatsSnapshotCandidateSet(replay, [CAPABILITY], heroStatsScan)[CAPABILITY];
  assert.deepEqual(reused, direct);
});

test('CLI capability preflight names the optional output and distinguishes missing tail', () => {
  const replay = fixture();
  const entry = capabilityQuery(replay).capabilities.find((row) => row.capability === CAPABILITY);
  assert.equal(entry.status, 'CANDIDATE');
  assert.equal(entry.output, `${CAPABILITY}_candidates`);
  assert.deepEqual(entry.missing_inputs, []);
