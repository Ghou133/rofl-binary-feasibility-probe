...
const {
  NPC_BUFF_UPDATE_COUNT_PACKET_CANDIDATE_PROFILE_821: profile,
  decodeNpcBuffUpdateCountPacketCandidates821: decode,
} = require('../src/decoders/rofl_16_19_821_buff_update_count_packet_candidate');
...
const PAYLOAD = Buffer.from('f545b203a97979b9f3', 'hex');

function packet(packetId = 0x02d9, rawParam = 0x400000ae, payload = PAYLOAD) {
...
function fakeImage(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-buff-update-count-'));
...
function nativeResult(request) {
  return {
    ...
    results: request.packets.map((input, inputIndex) => ({
      ...
      native_packet_id: 0x02d9, native_raw_param: input.raw_param,
      opaque_u8_0x10: 1, opaque_u8_0x11: 41,
      opaque_u32_0x14: 1073741998,
      opaque_f32_0x18: 0, opaque_f32_0x1c: 15.999999046325684,
      raw_u8_0x10_hex: '60', raw_u8_0x11_hex: 'b2',
      raw_u32_0x14_hex: '03bcbcd6',
      raw_f32_0x18_hex: 'f8f8f8f8', raw_f32_0x1c_hex: '7979b9f3',
    })),
  };
}

test('821 BuffUpdateCount exposes anonymous packet fields with source-bound shared scan', (t) => {
  const image = fakeImage(t);
  const replay = replayWithChunks([
    { packets: [packet()] },
    { packets: [packet(0x0194, 0x400000ae,
      Buffer.from('1d438b48370f35', 'hex')),
    packet(0x02d9, 0x400000b4, Buffer.alloc(15))] },
  ]);
  const invoke = t.mock.method(childProcess, 'spawnSync', (_python, args, options) => {
    assert.match(args[1], /decode_buff_update_count_packet_16_19_821\.py$/);
  ...
  const token = collect821Routes(replay, ['npc_buff_update_count_packet']);
  const result = decode(replay, { runtimeImagePath: image, precollected: token });
  assert.equal(profile.packet_name, 'PKT_NPC_BuffUpdateCount_s');
  assert.equal(result.status, 'CANDIDATE');
  assert.equal(result.input_count, 2);
  assert.equal(result.event_count, 2);
  assert.equal(result.runtime_image_status, 'MATCHED_USED');
  assert.equal(result.events[0].event_type, 'NPC_BUFF_UPDATE_COUNT_PACKET_CANDIDATE');
  assert.equal(result.events[0].opaque_u32_0x14, 1073741998);
  assert.equal(result.events[1].opaque_f32_0x1c, 15.999999046325684);
  assert.equal(result.events[0].raw_object_u8_0x10_hex, '60');
...
test('821 BuffUpdateCount separates absent, missing-input and unsupported build', () => {
...
test('821 BuffUpdateCount rejects altered source and foreign stream or length', (t) => {
  ...
  assert.equal(decode(replayWithChunks([{ packets: [packet(0x02d9, 0x400000ae,
...
test('821 BuffUpdateCount rejects wrong image, incomplete native output and digest drift', (t) => {
  const image = fakeImage(t);
  ...
  assert.equal(incomplete.first_failed_packet_ref.packet_id, 0x02d9);
...
test('821 BuffUpdateCount bounds direct and shared retention at 25,000 packets', (t) => {
  const invoke = t.mock.method(childProcess, 'spawnSync', () => {
    throw new Error('native decoder should not be invoked');
  });
...
test('821 BuffUpdateCount exact image decodes a pinned real packet', {
  skip: !fs.existsSync(IMAGE),
}, () => {
  const request = JSON.stringify({
    replay_version: BUILD,
    packets: [PAYLOAD, PAYLOAD.subarray(0, -1), Buffer.concat([PAYLOAD, Buffer.from([0xa5])])]
      .map((payload) => ({ packet_id: 0x02d9, stream_tag: 1, raw_param: 0x400000ae,
        payload_hex: payload.toString('hex') }))
      .concat({ packet_id: 0x0194, stream_tag: 1, raw_param: 0x400000ae,
        payload_hex: '1d438b48370f35' }),
  });
  const run = childProcess.spawnSync(process.env.PYTHON || 'python', [
    '-B', path.resolve(__dirname, '..', 'scripts',
      'decode_buff_update_count_packet_16_19_821.py'), '--image', IMAGE,
  ...
  assert.equal(row.opaque_u8_0x11, 41);
  assert.equal(row.opaque_u32_0x14, 1073741998);
  assert.equal(row.opaque_f32_0x18, 0);
  assert.equal(row.opaque_f32_0x1c, 15.999999046325684);
