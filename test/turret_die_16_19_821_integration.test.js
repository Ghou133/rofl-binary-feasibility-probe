...
const TARGET = payloadWithRawChildSuffix('247152269566');
const FOREIGN = payloadWithRawChildSuffix('247152269548');
...
test('821 OnTurretDie registry and shared scan expose only the exact candidate route', () => {
  const input = replay([TARGET, FOREIGN, Buffer.alloc(20)]);
  assert.equal(resolveBuildProfile(input).status, 'SUPPORTED');
  assert.equal(resolveCapability(BUILD, 'turret_die_event_packet').status, 'CANDIDATE');
  const queried = capabilityQuery(input).capabilities.find((row) =>
    row.capability === 'turret_die_event_packet');
  assert.equal(queried.runtime_image_requirement, 'EXACT_IMAGE_REQUIRED');
  assert.equal(queried.output, 'turret_die_event_packet_candidates');
  const scan = collect821Routes(input,
    ['turret_die_event_packet', 'revive_ally_event_packet']);
...
test('821 OnTurretDie API and CLI emit native target and retain foreign exclusions', (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-turret-die-entry-'));
  ...
  const missing = decodeSemanticReplay(input, {
    capabilities: ['turret_die_event_packet'],
  });
  assert.equal(missing.capability_results.turret_die_event_packet.status, 'MISSING_INPUT');
  ...
  assert.equal(decoded.capability_results.turret_die_event_packet.runtime_image_status,
    'MATCHED_USED');
  assert.equal(decoded.capability_results.turret_die_event_packet.excluded_same_length_foreign_count, 1);
  assert.equal(decoded.events.turret_die_event_packet_candidates.length, 1);

  const parsed = parseOne(filePath, {
    semantic: true, events: ['turret_die_event_packet'],
    runtimeImage: imagePath, strict: true, timelineLimit: 0,
  });
  assert.equal(parsed.ok, true);
  assert.equal(parsed.analysis.semantic.capability_results.turret_die_event_packet.status,
    'CANDIDATE');
  assert.deepEqual(Object.keys(parsed.analysis.events),
    ['turret_die_event_packet_candidates']);
  assert.equal(parsed.analysis.events.turret_die_event_packet_candidates[0].event_name,
    'OnTurretDie');
  ...
  assert.equal(absent.analysis.semantic.capability_results.turret_die_event_packet.status,
    'PROFILE_UNAVAILABLE');
  assert.equal(absent.analysis.semantic.capability_results.turret_die_event_packet.event_count,
    null);
