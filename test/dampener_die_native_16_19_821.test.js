...
function evidenceRows() {
  ...
  const target = rows.filter((row) => row.payload_length === 116
    && row.payload_hex.endsWith('247152269506')
    && row.payload_hex.slice(2, 10) === '1eb9b33d');
  assert.equal(rows.filter((row) => row.payload_length === 116).length, 831);
  assert.equal(target.length, 18);
  assert.equal(new Set(target.map((row) => row.replay_name)).size, 10);
  return target;
}
...
test('821 native OnDampenerDie helper fully decodes 18 observed child 0x0035 packets', {
  ...
  for (const [index, result] of output.results.entries()) {
    ...
    assert.equal(result.event_id, 0x0035);
    assert.equal(result.raw_event_id_hex, '0x4906');
    ...
    for (const field of ['actor', 'target', 'dampener', 'dampener_die',
...
test('821 native OnDampenerDie helper rejects all four real same-length foreign children atomically', {
  skip: !LOCAL_INPUTS ? 'requires exact-build image and ignored raw-row evidence' : false,
}, () => {
...
test('821 native OnDampenerDie helper rejects malformed route, build, payload, and image', {
  skip: !LOCAL_INPUTS ? 'requires exact-build image and ignored raw-row evidence' : false,
}, () => {
  const first = packet(evidenceRows()[0]);
  const controls = [
