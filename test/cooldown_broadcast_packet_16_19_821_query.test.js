...
function row(index) {
  ...
  return {
    ...
    native_protected_lookup_byte_hex: protectedHex,
    ...
    raw_packet_ref: {
      ...
      chunk_index: index, chunk_id: index + 1, chunk_stream: index === 0 ? 'game_chunk' : 'keyframe',
...
function hashes(rows) {
  ...
  for (const item of rows) {
    ...
    output.update(Buffer.from(item.native_protected_lookup_byte_hex, 'hex'))
      .update(key);
  }
...
function fixture(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-cooldown-query-'));
...
test('later forged native output is detected after limit with no partial output', (t) => {
  ...
  forged[1].native_protected_lookup_byte_hex = PACKETS[0][1];
