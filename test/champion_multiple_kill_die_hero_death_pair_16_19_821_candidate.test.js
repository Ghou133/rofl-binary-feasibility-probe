...
const { CHAMPION_MULTIPLE_KILL_DIE_HERO_DEATH_PAIR_821_PROFILE,
  associateChampionMultipleKillDieHeroDeathCandidates821: associate } =
...
function fixture(specs = [
  { timeMs: 1000, dieRaw: 0x400001ae, victimRaw: 0x400001ae,
    sourceId: 0x400000b0, multiKey: 0x400000ae, hasMulti: true },
  { timeMs: 2000, dieRaw: 0x400000af, victimRaw: 0x400000af,
    sourceId: 0x400000b1, multiKey: 0x400000af, hasMulti: true },
  { timeMs: 3000, dieRaw: 0x400000b2, victimRaw: 0x400000b2,
    sourceId: 0x400000b3, multiKey: 0x400000b2, hasMulti: false },
]) {
  const body = Buffer.concat([
    ...
    ...specs.flatMap((spec) => [
      ...
      ...(spec.hasMulti ? [packet(0x040a, spec.sourceId, 88,
        spec.multiTimeMs ?? spec.timeMs)] : []),
  ...
  for (const spec of specs) {
    ...
    if (spec.hasMulti) {
      const multiRef = get(0x040a, 88, spec.multiTimeMs ?? spec.timeMs);
      multiEvents.push({
        event_type: 'CHAMPION_MULTIPLE_KILL_EVENT_PACKET_CANDIDATE',
        game_version: BUILD,
        build_profile: CHAMPION_MULTIPLE_KILL_EVENT_PACKET_821_PROFILE.id,
        confidence: 'CANDIDATE',
        semantic_status: 'CANDIDATE_EXACT_RUNTIME_ON_CHAMPION_MULTIPLE_KILL_PACKET',
        replay_sha256: replay.source_sha256,
        replay_time_ms: multiRef.replay_time_ms,
        raw_param: spec.sourceId,
        event_id: 0x0009, event_name: 'OnChampionMultipleKill',
        raw_event_id_hex: '0x4989',
        event_u32_0x04: spec.multiKey,
        event_u32_0x08: 1,
        event_u32_0x0c: 1,
        event_u32_list_0x10: [0x400000b7],
        event_blob_sha256: 'b'.repeat(64),
        raw_packet_ref: multiRef,
      });
    }
  }
  return {
    replay,
    championMultipleKillEventPacketOutcome: {
      status: 'CANDIDATE',
      profile_id: CHAMPION_MULTIPLE_KILL_EVENT_PACKET_821_PROFILE.id,
      evidence_runtime_image_sha256: IMAGE_SHA256,
      runtime_image_sha256: IMAGE_SHA256, runtime_image_status: 'MATCHED_USED',
      runtime_image_used: true, input_packet_id: 0x040a, child_event_id: 0x0009,
      input_count: multiEvents.length, event_count: multiEvents.length,
      events: multiEvents,
    },
...
test('821 Multi packet group retains routes and counts Die/Hero_Die groups without Multi', () => {
  const values = fixture();
  const result = associate(values.replay, values);
  assert.equal(CHAMPION_MULTIPLE_KILL_DIE_HERO_DEATH_PAIR_821_PROFILE.capability,
    'champion_multiple_kill_die_hero_death_pair');
  assert.equal(result.status, 'CANDIDATE');
  assert.equal(result.pair_count, 2);
  assert.equal(result.unmatched_on_champion_multiple_kill_count, 0);
  assert.equal(result.unpaired_on_champion_die_count, 1);
  assert.equal(result.unpaired_hero_death_count, 1);
  assert.deepEqual(result.multi_0x04_xor_hero_raw_delta_counts,
  ...
  assert.equal(result.events[0].on_champion_multiple_kill_raw_param,
    result.events[0].on_champion_die_event_u32_0x04);
  assert.equal(result.events[0].on_champion_multiple_kill_raw_param,
    result.events[0].hero_death_die_source_network_id_candidate);
  assert.equal(result.events[0].on_champion_multiple_kill_event_u32_0x0c,
    result.events[0].on_champion_multiple_kill_event_u32_list_0x10.length);
  for (const field of ['killer_participant_id', 'victim_participant_id',
    'effective_multikill', 'effective_death', 'killing_spree']) {
    assert.equal(field in result.events[0], false);
  }
});

test('821 Multi packet group fails atomically on duplicated keys or packet positions', () => {
  const duplicate = fixture();
  const extra = structuredClone(duplicate.championMultipleKillEventPacketOutcome.events[0]);
  extra.raw_packet_ref.decompressed_block_offset += 1;
  extra.raw_packet_ref.decompressed_payload_offset += 1;
  duplicate.championMultipleKillEventPacketOutcome.events.push(extra);
  duplicate.championMultipleKillEventPacketOutcome.event_count += 1;
  duplicate.championMultipleKillEventPacketOutcome.input_count += 1;
  ...
  samePosition.championMultipleKillEventPacketOutcome.events[0].raw_packet_ref
...
test('821 Multi packet group fails atomically on missing or shifted counterpart', () => {
  const shifted = fixture([{ timeMs: 1000, multiTimeMs: 2000,
    dieRaw: 0x400001ae, victimRaw: 0x400001ae,
    sourceId: 0x400000b0, multiKey: 0x400000ae, hasMulti: true }]);
  const result = associate(shifted.replay, shifted);
  assert.equal(result.status, 'INCONSISTENT');
  assert.equal(result.events, null);
  assert.equal(result.diagnostics.unmatched_on_champion_multiple_kill_count, 1);
});

test('821 Multi packet group rejects packet order, field conflicts and malformed list', () => {
  const order = fixture();
  const multiRef = order.championMultipleKillEventPacketOutcome.events[0].raw_packet_ref;
  const dieOffset = order.championDieEventPacketOutcome.events[0]
    .raw_packet_ref.decompressed_block_offset;
  multiRef.decompressed_block_offset = dieOffset - 1;
  multiRef.decompressed_payload_offset = dieOffset + 1;
  ...
  source.championMultipleKillEventPacketOutcome.events[1].raw_param ^= 1;
  source.championMultipleKillEventPacketOutcome.events[1].raw_packet_ref.raw_param ^= 1;
  ...
  const field = fixture();
  field.championMultipleKillEventPacketOutcome.events[1].event_u32_0x04 ^= 0x200;
  assert.equal(associate(field.replay, field).status, 'INCONSISTENT');

  const list = fixture();
  list.championMultipleKillEventPacketOutcome.events[0].event_u32_0x0c = 2;
  assert.equal(associate(list.replay, list).status, 'INCONSISTENT');
});

test('821 Multi packet group requires exact upstream outcomes and source integrity', () => {
  const values = fixture();
  assert.equal(associate(values.replay, {
    championMultipleKillEventPacketOutcome: values.championMultipleKillEventPacketOutcome,
  }).status, 'MISSING_INPUT');
  const wrongImage = fixture();
  wrongImage.championMultipleKillEventPacketOutcome.runtime_image_sha256 = 'f'.repeat(64);
  ...
  assert.equal(associate(wrongDie.replay, wrongDie).status, 'INCONSISTENT');
  const foreign = fixture();
  foreign.championMultipleKillEventPacketOutcome.events[0].replay_sha256 = 'f'.repeat(64);
