...
const {
  CHAMPION_TRIPLE_QUADRA_EVENT_PACKET_821_PROFILE: profile,
  decodeChampionTripleQuadraEventPacketCandidates821: decode,
...
function fakeImage(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-triple-quadra-test-'));
...
function nativeResult(request, childIds) {
  return {
    ...
    results: request.packets.map((row, index) => {
      ...
      return {
        status: id === 0x000c || id === 0x000d ? 'DECODED' : 'EXCLUDED_CHILD',
...
test('821 TripleKill and QuadraKill select exact children among same-length controls', (t) => {
  ...
  const invoke = t.mock.method(childProcess, 'spawnSync', (_python, args, options) => {
    assert.match(args[1], /decode_champion_triple_quadra_event_packet_16_19_821\.py$/);
    ...
    return { status: 0, stderr: '',
      stdout: JSON.stringify(nativeResult(request, [0x000c, 0x0007, 0x000d, 0x000b])) };
  });
  const result = decode(replay, { runtimeImagePath: image });
  assert.deepEqual(profile.child_event_ids, [0x000c, 0x000d]);
  assert.deepEqual(profile.child_event_names,
    ['OnChampionTripleKill', 'OnChampionQuadraKill']);
  assert.equal(result.status, 'CANDIDATE');
  assert.equal(result.input_count, 4);
  assert.equal(result.target_packet_count, 2);
  assert.equal(result.excluded_child_count, 2);
  assert.deepEqual(result.excluded_child_ids,
    { '0x0007': 1, '0x000b': 1 });
  assert.equal(result.runtime_image_status, 'MATCHED_USED');
  const event = result.events[0];
  assert.equal(event.event_type, 'CHAMPION_TRIPLE_QUADRA_EVENT_PACKET_CANDIDATE');
  assert.equal(event.child_event_id, 0x000c);
  assert.equal(event.registered_event_name, 'OnChampionTripleKill');
  assert.equal(event.raw_event_id_hex, '0x49c8');
  assert.equal(event.event_blob_sha256, hash(Buffer.alloc(96, 0x0c)));
  assert.equal(event.raw_packet_ref.replay_sha256, replay.source_sha256);
  assert.equal(event.raw_packet_ref.packet_id, 0x040a);
  assert.equal(result.events[1].child_event_id, 0x000d);
  assert.equal(result.events[1].registered_event_name, 'OnChampionQuadraKill');
  assert.equal(result.events[1].raw_event_id_hex, '0x4988');
  assert.equal(result.events[1].event_blob_sha256, hash(Buffer.alloc(96, 0x0d)));
...
test('821 triple/quadra marker distinguishes absent shape, missing image, wrong build, and source corruption', () => {
...
test('821 triple/quadra marker rejects foreign stream and unknown native child', (t) => {
  const image = fakeImage(t);
...
test('821 triple/quadra marker reports fully decoded foreign children without target events', (t) => {
  const image = fakeImage(t);
  const replay = replayWithChunks([{ packets: [packet()] }]);
  t.mock.method(childProcess, 'spawnSync', (_python, _args, options) => {
    const request = JSON.parse(options.input);
...
test('821 triple/quadra marker fails atomically on tampered native identity and blob', (t) => {
  const image = fakeImage(t);
  const replay = replayWithChunks([{ packets: [packet(), packet(0x040a, 0x400000af)] }]);
  let tamper = 'event_blob_sha256';
  t.mock.method(childProcess, 'spawnSync', (_python, _args, options) => {
    const request = JSON.parse(options.input);
    const output = nativeResult(request, [0x000c, 0x0007]);
    ...
    else if (tamper === 'raw_event_id_hex') second.raw_event_id_hex = '0x49c8';
...
test('821 triple/quadra marker exposes wrong image and missing native dependency distinctly', (t) => {
  const image = fakeImage(t);
  const replay = replayWithChunks([{ packets: [packet()] }]);
