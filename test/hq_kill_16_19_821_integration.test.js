...
const TARGET = payloadWithRawChildSuffix('247152269518');
  ...
  return {
    ...
    results: request.packets.map((row, inputIndex) => {
      ...
      return {
        ...
        event_id: 0x0046, raw_event_id_hex: '0x4918',
...
test('821 OnHQKill registry and shared scan expose the exact candidate route', () => {
  const input = replay([TARGET, FOREIGN, Buffer.alloc(20)]);
  assert.equal(resolveBuildProfile(input).status, 'SUPPORTED');
  assert.equal(resolveCapability(BUILD, 'hq_kill_event_packet').status,
    'CANDIDATE');
  const queried = capabilityQuery(input).capabilities.find((row) =>
    row.capability === 'hq_kill_event_packet');
  assert.equal(queried.runtime_image_requirement, 'EXACT_IMAGE_REQUIRED');
  assert.equal(queried.output, 'hq_kill_event_packet_candidates');
  const scan = collect821Routes(input,
    ['hq_kill_event_packet', 'turret_die_event_packet']);
  assert.deepEqual(rowsFor821Capability(input, scan, 'hq_kill_event_packet')
    .rows.map((row) => row.block.payload_length), [116, 116]);
  assert.deepEqual(rowsFor821Capability(input, scan, 'turret_die_event_packet')
    .rows.map((row) => row.block.payload_length), [116, 116]);
});

test('821 OnHQKill API and CLI emit only native target and preserve controls', (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rofl-821-hq-kill-entry-'));
  ...
  const missing = decodeSemanticReplay(input, {
    capabilities: ['hq_kill_event_packet'],
  });
  assert.equal(missing.capability_results.hq_kill_event_packet.status,
    'MISSING_INPUT');
  assert.equal(missing.events, null);

  const native = t.mock.method(childProcess, 'spawnSync', (_python, args, options) => {
    assert.match(args[1], /decode_hq_kill_event_packet_16_19_821\.py$/);
  ...
  const decoded = decodeSemanticReplay(input, {
    capabilities: ['hq_kill_event_packet'], runtimeImagePath: imagePath,
  });
  assert.equal(decoded.status, 'EXPERIMENTAL_CANDIDATE');
  assert.equal(decoded.capability_results.hq_kill_event_packet.runtime_image_status,
    'MATCHED_USED');
  assert.equal(decoded.capability_results.hq_kill_event_packet
    .excluded_same_length_foreign_count, 1);
  assert.equal(decoded.events.hq_kill_event_packet_candidates.length, 1);

  const parsed = parseOne(filePath, {
    semantic: true, events: ['hq_kill_event_packet'],
    runtimeImage: imagePath, strict: true, timelineLimit: 0,
  });
  assert.equal(parsed.ok, true);
  assert.equal(parsed.analysis.semantic.capability_results.hq_kill_event_packet.status,
    'CANDIDATE');
  assert.deepEqual(Object.keys(parsed.analysis.events),
    ['hq_kill_event_packet_candidates']);
  assert.equal(parsed.analysis.events.hq_kill_event_packet_candidates[0].event_name,
    'OnHQKill');
  ...
  assert.equal(absent.analysis.semantic.capability_results.hq_kill_event_packet.status,
    'PROFILE_UNAVAILABLE');
  assert.equal(absent.analysis.semantic.capability_results.hq_kill_event_packet.event_count,
    null);
