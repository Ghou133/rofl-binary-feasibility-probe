...
generic_factory_callers = []
for call in factory_callers:
    owner = static.function_for(call.address - exhaust.IMAGE_BASE)
    instructions = static.disassemble_function(owner)
    generic_factory_callers.append({
        'factory_call': exhaust.instruction_row(call),
        'owner_begin_rva': owner[0],
        'owner_begin_rva_hex': f'0x{owner[0]:08x}',
        'owner_end_rva': owner[1],
        'owner_end_rva_hex': f'0x{owner[1]:08x}',
        'has_indirect_slot_1_call': any(
            instruction.mnemonic == 'call'
            and ('[rax + 8]' in instruction.op_str or '[rcx + 8]' in instruction.op_str)
...
for packet_id in route_ids:
    ...
    deserializer_entry = static.function_for(deserializer_rva)
    constructor_xrefs = static.direct_call_xrefs(constructor_rva)
    vtable_xrefs = static.lea_xrefs(vtable_rva)
    deserializer_xrefs = static.direct_call_xrefs(deserializer_rva)
    deserializer_targets = exhaust.direct_targets(static, deserializer_entry)
    slices = {
    ...
    routes[packet] = {
        ...
        'static_identity_xrefs': {
            'constructor_direct_call_xrefs': [
                exhaust.instruction_row(row) for row in constructor_xrefs
            ],
            'vtable_lea_xrefs': [exhaust.instruction_row(row) for row in vtable_xrefs],
            'deserializer_direct_call_xrefs': [
                exhaust.instruction_row(row) for row in deserializer_xrefs
            ],
            'deserializer_direct_call_targets': deserializer_targets,
            'deserializer_direct_call_targets_hex': [
                f'0x{value:08x}' for value in deserializer_targets
            ],
            'all_direct_xref_surfaces_scanned': True,
        },
        ...
        'full_pdata_immediate_reference_scan': {
            ...
            'candidate_function_rva_inventory_complete': True,
            'non_constructor_retained_matches': non_constructor,
            'retained_match_limit': scan['retained_match_limit'],
            'bounded_disposition': {
                'status': 'NO_UNIQUE_PACKET_CALLBACK_OR_RTTI_EDGE',
                'basis': (
                    'Every PE exception-directory function range was decoded; every matching '
                    'function RVA is retained. Small packet-ID immediates collide with arithmetic, '
                    'control-flow, and generic data constants. None closes a unique edge from the '
                    'packet factory/deserializer xrefs to a MakeFunction RTTI registration or '
                    'packet-specific receive target.'
                ),
                'semantic_claim': None,
            },
        },
        ...
        'validations': {
            ...
            'direct_xref_surfaces_scanned': True,
...
for left_index, left_key in enumerate(route_keys):
    left = routes[left_key]['factory_chain']
    for right_key in route_keys[left_index + 1:]:
        right = routes[right_key]['factory_chain']
        constructor_delta = right['constructor_rva'] - left['constructor_rva']
        vtable_delta = right['vtable_rva'] - left['vtable_rva']
        deserializer_delta = right['deserializer_rva'] - left['deserializer_rva']
        pairwise.append({
            'left_route': left_key,
            'right_route': right_key,
            'same_object_size': left['object_size'] == right['object_size'],
            'constructor_delta': constructor_delta,
            'constructor_delta_hex': hex(constructor_delta),
            'vtable_delta': vtable_delta,
            'vtable_delta_hex': hex(vtable_delta),
            'deserializer_delta': deserializer_delta,
            'deserializer_delta_hex': hex(deserializer_delta),
            'near_constructor_within_0x100': abs(constructor_delta) <= 0x100,
            'near_vtable_within_0x100': abs(vtable_delta) <= 0x100,
            'near_deserializer_within_0x400': abs(deserializer_delta) <= 0x400,
            'semantic_claim': None,
        })
relationships = {
    'pairwise_exact_rva_relationships': pairwise,
    'pair_count': len(pairwise),
    'all_pairs_examined': len(pairwise) == len(route_keys) * (len(route_keys) - 1) // 2,
    'semantic_claim': None,
}
result = {
    'schema': 'RESIDUAL_P5_STATIC_RUNTIME_RECOVERY_V2',
    'schema_version': 2,
    ...
    'factory': {
        ...
        'generic_factory_callers': generic_factory_callers,
    },
    'pdata_scan': {
    ...
    'all_direct_xref_surfaces_scanned': all(
        row['validations']['direct_xref_surfaces_scanned'] for row in routes.values()
    ),
...
function parseArgs(argv = process.argv.slice(2)) {
  const options = {
    ...
    outputDir: path.join(ROOT, 'artifacts', 'full_semantic_deep_recovery_v2', 'residual_p5_wave'),
    python: 'python',
    jobs: 4,
  };
...
async function staticRecovery(options) {
  ...
  invariant(parsed.all_direct_xref_surfaces_scanned === true,
    'static identity direct-xref scan incomplete');
...
function hashManifest(files) {
  return {
    schema: 'RESIDUAL_P5_WAVE_HASH_MANIFEST_V2',
    schema_version: 2,
...
async function audit(options = parseArgs()) {
  ...
  const staticPath = writeJson(path.join(options.outputDir, 'residual_p5_static_runtime_16_16.json'), staticReport);
  ...
  const corpus = writeJsonl(path.join(options.outputDir, 'residual_p5_distinct_payloads_16_16.jsonl'),
  ...
  const decisionBundle = {
    schema: 'RESIDUAL_P5_WAVE_MACHINE_DECISIONS_V2',
    schema_version: 2,
    analyzer_version: 'residual-p5-wave-v2',
  ...
  const validations = {
    ...
    all_static_identity_direct_xref_surfaces_scanned:
      staticReport.all_direct_xref_surfaces_scanned,
  ...
  invariant(validations.all_pass, `residual P5 validation failure: ${JSON.stringify(validations)}`);
  const report = {
    schema: 'RESIDUAL_P5_WAVE_DEEP_RECOVERY_V2',
    schema_version: 2,
    analyzer_version: 'residual-p5-wave-v2',
    generated_at: '2026-08-20',
    exact_build: EXACT_BUILD,
    ...
    methods_executed: [
      ...
      'CONSTRUCTOR_VTABLE_DESERIALIZER_DIRECT_XREF_AND_TARGET_SCAN',
    ...
    saturation: {
      ...
      closed_surfaces: [
        ...
        'CONSTRUCTOR_VTABLE_DESERIALIZER_DIRECT_XREF_SURFACE',
  ...
  const reportPath = writeJson(path.join(options.outputDir, 'residual_p5_wave_audit_16_16.json'), report);
  const decisionsPath = writeJson(path.join(options.outputDir, 'residual_p5_wave_decisions_16_16.json'), decisionBundle);
  const determinismPath = writeJson(path.join(options.outputDir, 'residual_p5_wave_determinism_16_16.json'), nativeDeterminism);
  const markdownPath = rejectProtectedPath(path.join(options.outputDir, 'residual_p5_wave_audit_16_16.md'));
  fs.writeFileSync(markdownPath, markdownReport(report), 'utf8');
  const artifactFiles = [
    path.join(ROOT, 'src', 'residual_p5_wave_v2.js'),
    path.join(ROOT, 'scripts', 'audit_residual_p5_wave_v2.js'),
    path.join(ROOT, 'test', 'residual_p5_wave_v2.test.js'),
  ...
  const hashesPath = writeJson(path.join(options.outputDir, 'residual_p5_wave_hashes_16_16.json'),
