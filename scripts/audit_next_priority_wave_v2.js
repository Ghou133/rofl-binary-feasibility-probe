...
function parseArgs(argv = process.argv.slice(2)) {
  const options = {
    ...
    outputDir: path.join(ROOT, 'artifacts', 'full_semantic_deep_recovery_v2', 'next_priority_wave'),
    python: 'python',
    jobs: 4,
...
function compareNativePasses(first, second, executedTwice) {
  ...
  const routes = first.map((left) => {
    ...
    const decodedMatch = Boolean(right && left.decoded_sha256 === right.decoded_sha256);
    const summaryMatch = Boolean(right && left.summary_sha256 === right.summary_sha256);
    return {
...
function sourceCrossCheck(registry, callbackSidecar, profiler, priorMining) {
  ...
  return ROUTE_IDS.map((packetId) => {
    ...
    const profileRoute = profilerRoutes.find((row) => Number(row.packet_id) === packetId);
    const miningRoute = priorMining.routes.find((row) => Number(row.packet_id) === packetId);
    invariant(registryRoute && sidecarRoute, `prior runtime evidence missing ${packetType}`);
    const registryNames = [...new Set(
      registryRoute.runtime_registration.callbacks.map((row) => row.name),
    )].sort();
    ...
    return {
      ...
      observed_count: registryRoute.observed.count,
      callback_names: registryNames,
      callback_sidecar_match: true,
      prior_profiler_route: profileRoute || null,
      prior_profiler_ranked_candidates: ranked.filter((row) => Number(row.packet_id) === packetId),
      prior_sampled_mining_route: miningRoute || null,
      conclusion: profileRoute || miningRoute
        ? 'PRIOR_CANDIDATE_EVIDENCE_REEXECUTED_WITH_FULL_INVENTORY_AND_EXACT_RUNTIME'
        : 'PRIOR_SURFACES_HAD_NO_ROUTE_ENTRY; THIS_WAVE_EXECUTED_THE_MISSING_EXACT_ROUTE',
    };
  });
}
...
function hashManifest(root, outputDir, files) {
  ...
  return {
    schema: 'NEXT_PRIORITY_WAVE_HASH_MANIFEST_V2',
    schema_version: 2,
    exact_build: EXACT_BUILD,
    excluded_self_hash: path.relative(root, path.join(outputDir, 'hashes_16_16.json')).replaceAll('\\', '/'),
    verification_commands: [
      'node --test test/next_priority_wave_v2.test.js',
      'node --max-old-space-size=4096 scripts/audit_next_priority_wave_v2.js',
    ],
...
async function audit(options = parseArgs()) {
  ...
  const callbackBytes = fs.readFileSync(options.callbackSidecar);
  const profilerBytes = fs.readFileSync(options.profiler);
  const priorMiningBytes = fs.readFileSync(options.priorMining);
  const image = fs.readFileSync(options.runtime);
  invariant(sha256(image) === RUNTIME_SHA256, 'pinned runtime image SHA-256 mismatch');
  ...
  writeJson(path.join(options.outputDir, 'source_cross_check_16_16.json'), {
    schema: 'NEXT_PRIORITY_WAVE_SOURCE_CROSS_CHECK_V2',
    schema_version: 2,
    exact_build: EXACT_BUILD,
    inputs: {
  ...
  const secondNativePass = options.verifyNativeDeterminism
    ...
    : firstNativePass;
  const nativeDeterminism = compareNativePasses(
    firstNativePass,
    secondNativePass,
  ...
  const report = buildReport({
    evidence,
    registryPath: path.relative(ROOT, options.registry).replaceAll('\\', '/'),
    runtimePath: path.relative(ROOT, options.runtime).replaceAll('\\', '/'),
    runtimeIdentities,
    nativeAnalysis,
    nativeDeterminism: {
  ...
  report.inputs.callback_sidecar = {
    path: path.relative(ROOT, options.callbackSidecar).replaceAll('\\', '/'),
    sha256: sha256(callbackBytes),
  };
  report.inputs.prior_profiler = {
    path: path.relative(ROOT, options.profiler).replaceAll('\\', '/'),
    sha256: sha256(profilerBytes),
  };
  report.inputs.prior_sampled_mining = {
    path: path.relative(ROOT, options.priorMining).replaceAll('\\', '/'),
    sha256: sha256(priorMiningBytes),
  };
  report.prior_evidence_cross_check = priorCrossCheck;
  invariant(report.validations.all_pass, `audit validation failure: ${JSON.stringify(report.validations)}`);
  const reportPath = writeJson(path.join(options.outputDir, 'next_priority_wave_audit_16_16.json'), report);
  const decisionsPath = writeJson(path.join(options.outputDir, 'next_priority_wave_machine_decisions_16_16.json'), {
    schema: 'NEXT_PRIORITY_WAVE_MACHINE_DECISIONS_V2',
    schema_version: 2,
    exact_build: EXACT_BUILD,
  ...
  const markdownPath = rejectProtectedPath(path.join(options.outputDir, 'next_priority_wave_audit_16_16.md'));
  ...
  const determinismPath = writeJson(path.join(options.outputDir, 'determinism_16_16.json'), {
    schema: 'NEXT_PRIORITY_WAVE_DETERMINISM_V2',
    schema_version: 2,
    exact_build: EXACT_BUILD,
    ...report.determinism,
  });
  const artifactFiles = [
    path.join(ROOT, 'src', 'next_priority_wave_v2.js'),
    path.join(ROOT, 'scripts', 'audit_next_priority_wave_v2.js'),
    path.join(ROOT, 'test', 'next_priority_wave_v2.test.js'),
    options.registry,
    ...
    options.runtime,
    corpus.path,
    path.join(options.outputDir, 'source_cross_check_16_16.json'),
...
async function main(argv = process.argv.slice(2)) {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    kda_terminal_match: `${result.report.independent_kda_validation.terminal_metadata_match_count}/${result.report.independent_kda_validation.terminal_metadata_check_count}`,
