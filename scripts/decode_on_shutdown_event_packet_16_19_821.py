#!/usr/bin/env python3
"""Decode exact 821 OnEvent child 0x00e8 candidate packets with the pinned image.

The image's OnEvent constructor/deserializer and callback child-ID transform
own decoding. OnShutdown is the exact image name-table label; the receiving
closure remains unresolved, so no gameplay effect or field role is assigned.
"""
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnShutdown request exceeds bounded input size')
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('OnShutdown candidate requires observed game stream')
    ...
    if not isinstance(payload_hex, str) or len(payload_hex) != PAYLOAD_LENGTH * 2:
        raise ValueError('payload_hex must encode one observed 105-byte OnEvent packet')
...
def decode_packet(emulator, context, raw_param, payload):
    ...
    if event_id != EVENT_ID or raw_event_id != RAW_EVENT_ID:
        raise ValueError(f'native OnEvent child ID differs from 0x00e8: 0x{event_id:04x}')
    ...
    try:
        ...
        try:
            ...
            raise ValueError(str(error).replace('ParamsHeal runtime image',
                                                'OnShutdown runtime image')) from error
        ...
        print(f'821 OnShutdown exact-runtime decoder error: {error}', file=sys.stderr)
