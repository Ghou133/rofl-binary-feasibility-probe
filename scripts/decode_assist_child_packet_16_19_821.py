#!/usr/bin/env python3
"""Decode exact 821 OnEvent assist-child packet fields as anonymous candidates.

The pinned client image supplies the OnEvent constructor, deserializer, and
child-ID transform. Its registered child handlers read +0x04 for both children
and +0x20 for child 0x0057. No actor or assist-effect role is inferred here.
"""
...
from decode_params_heal_packet_16_19_821 import (
    IMAGE_BASE, IMAGE_SHA256, PROFILE, VTABLE_RVA,
...
PACKET_ID = 0x040a
CHILD_IDS = frozenset((0x0056, 0x0057))
PAYLOAD_LENGTH = 44
BLOB_LENGTH = 36
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('assist-child request exceeds bounded input size')
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('assist-child candidate requires observed game stream')
    ...
    if not isinstance(payload_hex, str) or len(payload_hex) != PAYLOAD_LENGTH * 2:
        raise ValueError('payload_hex must encode one observed 44-byte OnEvent packet')
...
def check_image_identity(image):
    ...
    registrations = (
        (0x0056, 0x272003, 0x27200a, 0x2b8d20, 0x1ef80a0, b'OnDeathAssist'),
        (0x0057, 0x272020, 0x272027, 0x2b8e20, 0x1ef80c8, b'OnDeathAssistPost'),
    )
    for event_id, handler_lea, id_write, handler, name_entry, name in registrations:
        if (image[handler_lea:handler_lea + 3] != b'\x4c\x8d\x05'
                or handler_lea + 7 + struct.unpack_from('<i', image, handler_lea + 3)[0] != handler
                or image[id_write:id_write + 8] != b'\xc7\x44\x24\x20' + struct.pack('<I', event_id)):
            raise ValueError(f'exact 821 child 0x{event_id:04x} registration differs')
        name_pointer = struct.unpack_from('<Q', image, name_entry)[0] - IMAGE_BASE
        if name_pointer < 0 or image[name_pointer:name_pointer + len(name) + 1] != name + b'\0':
            raise ValueError(f'exact 821 child 0x{event_id:04x} name table differs')
    if (image[0x2b8d2a:0x2b8d2d] != b'\x8b\x52\x04'
            or image[0x2b8e35:0x2b8e38] != b'\x8b\x52\x04'
            or image[0x2b8e9a:0x2b8e9d] != b'\x8b\x56\x20'):
        raise ValueError('exact 821 assist-child callback field reads differ')
...
def read_image(path):
    ...
    if digest != IMAGE_SHA256 or len(image) != EXPECTED_IMAGE_BYTES:
        raise ValueError(f'assist-child runtime image SHA-256 mismatch: {digest}')
...
def decode_packet(emulator, context, raw_param, payload):
    ...
    if (native['deserialize_return_al'] != 1 or not native['fully_consumed']
        ...
        raise ValueError('native OnEvent deserializer did not fully consume 44-byte payload')
    ...
    if (count != BLOB_LENGTH or capacity != BLOB_LENGTH
        ...
        raise ValueError('native OnEvent child blob differs from observed 36-byte shape')
    ...
    if event_id not in CHILD_IDS:
        raise ValueError(f'native OnEvent child ID is outside 0x0056/0x0057: 0x{event_id:04x}')
    ...
    result = {
        ...
        'event_u32_0x04': struct.unpack_from('<I', blob, 4)[0],
    }
    if event_id == 0x0057:
        result['event_u32_0x20'] = struct.unpack_from('<I', blob, 0x20)[0]
    return result
...
def main():
    ...
    try:
        ...
        print(f'821 assist-child exact-runtime decoder error: {exc}', file=sys.stderr)
