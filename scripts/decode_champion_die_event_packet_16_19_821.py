#!/usr/bin/env python3
"""Decode exact 821 OnEvent child 0x0004 packet candidates with native code.

The parent constructor/deserializer and child-ID transform run against the
SHA-pinned 821 client image. The event name is the image's ID-indexed label;
neither it nor the direct child +0x04 read proves a death or object role.
"""
...
PACKET_ID = 0x040a
EVENT_ID = 0x0004
SAME_LENGTH_CONTROL_IDS = frozenset((0x0035, 0x003b, 0x003d, 0x0046))
PAYLOAD_LENGTH = 116
BLOB_LENGTH = 108
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnChampionDie request exceeds bounded input size')
...
def read_image(path):
    ...
    if digest != IMAGE_SHA256 or len(image) != 48488448:
        raise ValueError(f'OnChampionDie runtime image SHA-256 mismatch: {digest}')
    return image, digest
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('OnChampionDie candidate requires observed game stream')
...
def decode_packet(emulator, context, raw_param, payload):
    ...
    if event_id != EVENT_ID and event_id not in SAME_LENGTH_CONTROL_IDS:
        raise ValueError(f'unexpected length-116 OnEvent child ID: 0x{event_id:04x}')
    blob = bytes(emulator.emulator.mem_read(pointer, count))
    result = {
        'status': 'DECODED' if event_id == EVENT_ID else 'CONTROL',
    ...
    if event_id == EVENT_ID:
        result['event_u32_0x04'] = struct.unpack_from('<I', blob, 4)[0]
    return result
...
def main():
    ...
    try:
        ...
        print(f'821 OnChampionDie exact-runtime decoder error: {exc}', file=sys.stderr)
