#!/usr/bin/env python3
"""Decode exact-821 OnEvent child 0x0035 as a packet-local candidate.

OnDampenerDie is the pinned image name-table label. The 108-byte child blob is
left anonymous: packet identity does not establish a dampener state transition,
actor, target, or gameplay effect.
"""
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnDampenerDie request exceeds bounded input size')
...
def check_image_identity(image):
    ...
    name = b'OnDampenerDie\0'
    if name_rva < 0 or image[name_rva:name_rva + len(name)] != name:
        raise ValueError('exact 821 child 0x0035 name table differs')
...
def read_image(path):
    ...
    if digest != IMAGE_SHA256 or len(image) != EXPECTED_IMAGE_BYTES:
        raise ValueError(f'OnDampenerDie runtime image SHA-256 mismatch: {digest}')
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('OnDampenerDie candidate requires observed game stream')
...
def main():
    ...
    try:
        ...
        print(f'821 OnDampenerDie exact-runtime decoder error: {exc}', file=sys.stderr)
