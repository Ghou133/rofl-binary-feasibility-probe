#!/usr/bin/env python3
"""Decode exact-821 OnEvent child 0x0107 as a packet-local candidate.

The pinned client image owns parent deserialization and the child-ID transform.
Its name table calls the child OnTurretPlateDestroyed. The variable blob u32
is anonymous; neither a plate destruction nor an object role is established.
"""
...
PACKET_ID = 0x040a
EVENT_ID = 0x0107
OBSERVED_CONTROL_IDS = {0x00cb, 0x00cf, 0x0101, 0x0102, 0x0113, 0x0114, 0x0115}
RAW_EVENT_ID = 0x09e8
PAYLOAD_LENGTH = 17
BLOB_LENGTH = 8
OBSERVED_BLOB_U32_0X00 = 469
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnTurretPlateDestroyed request exceeds bounded input size')
...
def read_image(path):
    ...
    if digest != IMAGE_SHA256 or len(image) != 48488448:
        raise ValueError(f'OnTurretPlateDestroyed runtime image SHA-256 mismatch: {digest}')
    return image, digest
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('OnTurretPlateDestroyed candidate requires observed game stream')
...
def decode_packet(emulator, context, raw_param, payload):
    ...
    if event_id != EVENT_ID and event_id not in OBSERVED_CONTROL_IDS:
        raise ValueError(f'unobserved length-17 OnEvent child ID: 0x{event_id:04x}')
    if event_id == EVENT_ID and raw_event_id != RAW_EVENT_ID:
        raise ValueError(f'target raw OnEvent ID differs: 0x{raw_event_id:04x}')
    blob = bytes(emulator.emulator.mem_read(pointer, count))
    schema, first = struct.unpack('<II', blob)
    if event_id == EVENT_ID and schema != OBSERVED_BLOB_U32_0X00:
        raise ValueError(f'child +0x00 differs from observed {OBSERVED_BLOB_U32_0X00}')
    return {
...
def main():
    ...
    try:
        ...
        print(f'821 OnTurretPlateDestroyed exact-runtime decoder error: {exc}', file=sys.stderr)
