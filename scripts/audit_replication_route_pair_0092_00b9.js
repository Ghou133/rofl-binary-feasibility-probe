...
const {
  ...
  PREFIX_PACKET_ID,
  analyzePairEvents,
} = require('../src/replication_pair_audit');
...
function parseArgs(argv) {
  const options = {
    ...
    outputPath: 'artifacts/full_semantic_deep_recovery_v2/unknown_mining/route_pair_0092_00b9_exact_audit.json',
  };
...
function selectedEvent(replay, block, chunk) {
  return {
...
function main(argv = process.argv.slice(2)) {
  ...
  for (const packetId of [PREFIX_PACKET_ID, PAYLOAD_PACKET_ID]) {
  ...
  for (const replayPath of options.replayFiles) {
    ...
    const walk = walkBlocks(replay, (block, chunk) => {
      if (![PREFIX_PACKET_ID, PAYLOAD_PACKET_ID].includes(block.packet_id)) return;
      events.push(selectedEvent(replay, block, chunk));
  ...
  const report = analyzePairEvents(events, expectedCounts);
  report.input = {
    replay_count: replays.length,
    replays,
    registry_path: path.resolve(options.registryPath),
    registry_sha256: crypto.createHash('sha256').update(fs.readFileSync(options.registryPath)).digest('hex'),
  };
...
module.exports = { main, parseArgs, selectedEvent };
