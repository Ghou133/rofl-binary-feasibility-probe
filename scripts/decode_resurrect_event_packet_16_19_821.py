#!/usr/bin/env python3
"""Decode exact-821 OnEvent child 0x002d as a packet-local candidate.

The pinned client image owns parent deserialization and the child-ID transform.
Its name table calls the child OnResurrect. The two exposed blob u32 values are
anonymous; neither a resurrection nor any object role is established here.
"""
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnResurrect request exceeds bounded input size')
...
def read_image(path):
    ...
    if digest != IMAGE_SHA256 or len(image) != 48488448:
        raise ValueError(f'OnResurrect runtime image SHA-256 mismatch: {digest}')
    return image, digest
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('OnResurrect candidate requires observed game stream')
    ...
    if not isinstance(payload_hex, str) or len(payload_hex) != PAYLOAD_LENGTH * 2:
        raise ValueError('payload_hex must encode one observed 20-byte OnEvent packet')
...
def decode_packet(emulator, context, raw_param, payload):
    ...
    if count != BLOB_LENGTH or capacity != BLOB_LENGTH \
        ...
        raise ValueError('native OnEvent child blob differs from observed 12-byte shape')
    ...
    if event_id != EVENT_ID or raw_event_id != RAW_EVENT_ID:
        raise ValueError(f'unexpected length-20 OnEvent child/raw ID: 0x{event_id:04x}/0x{raw_event_id:04x}')
    blob = bytes(emulator.emulator.mem_read(pointer, count))
    schema, first, second = struct.unpack('<III', blob)
    if schema != OBSERVED_BLOB_U32_0X00:
    ...
    return {
        ...
        'event_u32_0x04': first, 'event_u32_0x08': second,
    }
...
def main():
    ...
    try:
        ...
        print(f'821 OnResurrect exact-runtime decoder error: {exc}', file=sys.stderr)
