#!/usr/bin/env python3
"""Decode exact-821 OnEvent child 0x0113 as a packet-local candidate.

The pinned client image owns parent deserialization and the child-ID transform.
Its name table calls the child OnObjectiveBountyClaimed. The variable blob u32
is anonymous; an object role or game-state effect is not established.
"""
...
PACKET_ID = 0x040a
EVENT_ID = 0x0113
OBSERVED_CONTROL_IDS = {0x00cb, 0x00cf, 0x0101, 0x0102, 0x0107, 0x0114, 0x0115}
RAW_EVENT_ID = 0x09e5
PAYLOAD_LENGTH = 17
BLOB_LENGTH = 8
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnObjectiveBountyClaimed request exceeds bounded input size')
...
def read_image(path):
    ...
    if digest != IMAGE_SHA256 or len(image) != 48488448:
        raise ValueError(f'OnObjectiveBountyClaimed runtime image SHA-256 mismatch: {digest}')
    return image, digest
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('OnObjectiveBountyClaimed candidate requires observed game stream')
...
def main():
    ...
    try:
        ...
        print(f'821 OnObjectiveBountyClaimed exact-runtime decoder error: {exc}', file=sys.stderr)
