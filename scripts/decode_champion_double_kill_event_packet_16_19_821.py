#!/usr/bin/env python3
"""Normalize exact-821 native 104-byte OnEvent rows for child 0x000b.
...
selects the exact-image OnChampionDoubleKill-named child without interpreting
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnChampionDoubleKill request exceeds bounded input size')
...
def main():
    ...
    try:
        ...
        for index, packet in enumerate(packets):
            ...
            row['status'] = 'DECODED' if child == TARGET_ID else 'EXCLUDED_CHILD'
        ...
        print(f'821 OnChampionDoubleKill exact-runtime decoder error: {error}',
              file=sys.stderr)
