#!/usr/bin/env python3
"""Decode exact-821 OnEvent child 0x003d as a packet-local candidate.

OnTurretFirstBlood is the pinned image name-table label. The 108-byte child
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnTurretFirstBlood request exceeds bounded input size')
...
def check_image_identity(image):
    ...
    name = b'OnTurretFirstBlood\0'
    if name_rva < 0 or image[name_rva:name_rva + len(name)] != name:
        raise ValueError('exact 821 child 0x003d name table differs')
...
def read_image(path):
    ...
    if digest != IMAGE_SHA256 or len(image) != EXPECTED_IMAGE_BYTES:
        raise ValueError(f'OnTurretFirstBlood runtime image SHA-256 mismatch: {digest}')
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('OnTurretFirstBlood candidate requires observed game stream')
...
def main():
    ...
    try:
        ...
        print(f'821 OnTurretFirstBlood exact-runtime decoder error: {exc}', file=sys.stderr)
