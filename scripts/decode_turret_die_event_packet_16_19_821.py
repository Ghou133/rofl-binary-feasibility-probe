#!/usr/bin/env python3
"""Decode exact-821 OnEvent child 0x003b as a packet-local candidate.

OnTurretDie is the pinned image name-table label. The 108-byte child blob is
left anonymous: packet identity does not establish a turret state transition,
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnTurretDie request exceeds bounded input size')
...
def check_image_identity(image):
    ...
    name = b'OnTurretDie\0'
    if name_rva < 0 or image[name_rva:name_rva + len(name)] != name:
        raise ValueError('exact 821 child 0x003b name table differs')
...
def read_image(path):
    ...
    if digest != IMAGE_SHA256 or len(image) != EXPECTED_IMAGE_BYTES:
        raise ValueError(f'OnTurretDie runtime image SHA-256 mismatch: {digest}')
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('OnTurretDie candidate requires observed game stream')
...
def main():
    ...
    try:
        ...
        print(f'821 OnTurretDie exact-runtime decoder error: {exc}', file=sys.stderr)
