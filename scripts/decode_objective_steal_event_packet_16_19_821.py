#!/usr/bin/env python3
"""Classify exact-821 OnEvent objective-steal child packets.

The pinned image establishes the two child IDs and their name-table labels.
The 124-byte child blob remains opaque; no objective kill, steal, actor, or
game-state effect is inferred from a packet.
"""
...
CHILD_IDS = {
    0x00be: ('OnKillDragonSteal', 0x499e),
    0x00d6: ('OnKillWormSteal', 0x490c),
}
...
def verify_image_route(image):
    ...
    for child_id, (name, raw_id) in CHILD_IDS.items():
        ...
        decoded = (decode_event_id_byte(raw_id & 0xff)
                   | (decode_event_id_byte(raw_id >> 8) << 8))
        if decoded != child_id:
            raise ValueError(f'exact 821 child 0x{child_id:04x} encoded ID differs')


def read_request():
    raw = sys.stdin.buffer.read(MAX_INPUT_BYTES + 1)
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('objective-steal request exceeds bounded input size')
...
def validate_packet(packet):
    ...
    if not isinstance(payload_hex, str) or len(payload_hex) != PAYLOAD_LENGTH * 2 \
        ...
        raise ValueError('payload_hex must encode one 133-byte OnEvent packet')
    return raw_param, bytes.fromhex(payload_hex)


def decode_packet(machine, context, raw_param, payload):
    if len(payload) != PAYLOAD_LENGTH:
        raise ValueError('expected exact 133-byte OnEvent payload')
    ...
    if native['deserialize_return_al'] != 1 or not native['fully_consumed'] \
        ...
        raise ValueError('native OnEvent deserializer did not fully consume 133 bytes')
    ...
    if count != BLOB_LENGTH or capacity != BLOB_LENGTH \
        ...
        raise ValueError('native OnEvent child blob differs from observed 124-byte scope')
    ...
    if child_id not in CHILD_IDS or raw_id != CHILD_IDS[child_id][1]:
        raise ValueError(f'unexpected length-133 OnEvent child/raw ID 0x{child_id:04x}/0x{raw_id:04x}')
    ...
    return {
        ...
        'event_id': child_id,
        'raw_event_id_hex': f'0x{raw_id:04x}',
...
def main():
    ...
    try:
        ...
        print(f'821 objective-steal exact-runtime decoder error: {error}', file=sys.stderr)
