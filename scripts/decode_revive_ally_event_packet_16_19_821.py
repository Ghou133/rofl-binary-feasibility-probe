#!/usr/bin/env python3
"""Decode exact-821 OnEvent child 0x002c as a packet-local candidate.

The pinned client image owns the parent deserializer and child-ID transform.
OnReviveAlly is the exact image name-table label. The native child blob's
+0x04 dword remains anonymous; no revival effect or actor role is inferred.
"""
...
def read_request():
    ...
    if len(raw) > MAX_INPUT_BYTES:
        raise ValueError('OnReviveAlly request exceeds bounded input size')
...
def check_image_identity(image):
    ...
    handler_lea = 0x272534
    if (image[handler_lea:handler_lea + 3] != b'\x4c\x8d\x05'
            or handler_lea + 7 + struct.unpack_from('<i', image, handler_lea + 3)[0] != 0x2d1590
            or image[0x27253b:0x272543] != b'\xc7\x44\x24\x20\x2c\x00\x00\x00'
            or image[0x272543:0x272551] !=
            b'\x48\x8d\x54\x24\x20\x48\x8d\x4f\x48\xe8\x4f\xa2\x06\x00'):
        raise ValueError('exact 821 child 0x002c registration differs')
    name_entry = 0x1ef7330 + EVENT_ID * 0x28
    name_rva = struct.unpack_from('<Q', image, name_entry)[0] - IMAGE_BASE
    name = b'OnReviveAlly\0'
    if name_rva < 0 or image[name_rva:name_rva + len(name)] != name:
        raise ValueError('exact 821 child 0x002c name table differs')
    if (image[0x2d15a4:0x2d15ab] != b'\x48\x8b\x52\x08\x8b\x52\x04'
            or image[0x2d15d6:0x2d15d9] != b'\x8b\x57\x04'):
        raise ValueError('exact 821 child 0x002c callback +0x04 reads differ')
...
def read_image(path):
    ...
    if digest != IMAGE_SHA256 or len(image) != EXPECTED_IMAGE_BYTES:
        raise ValueError(f'OnReviveAlly runtime image SHA-256 mismatch: {digest}')
...
def validate_packet(packet):
    ...
    if packet.get('stream_tag') != 1:
        raise ValueError('OnReviveAlly candidate requires observed game stream')
    ...
    if not isinstance(payload_hex, str) or len(payload_hex) != PAYLOAD_LENGTH * 2:
        raise ValueError('payload_hex must encode one observed 16-byte OnEvent packet')
...
def decode_packet(emulator, context, raw_param, payload):
    ...
    if (native['deserialize_return_al'] != 1 or not native['fully_consumed']
        ...
        raise ValueError('native OnEvent deserializer did not fully consume 16-byte payload')
    ...
    if event_id != EVENT_ID or raw_event_id != RAW_EVENT_ID:
        raise ValueError(f'unexpected length-16 OnEvent child/raw ID: 0x{event_id:04x}/0x{raw_event_id:04x}')
...
def main():
    ...
    try:
        ...
        print(f'821 OnReviveAlly exact-runtime decoder error: {exc}', file=sys.stderr)
