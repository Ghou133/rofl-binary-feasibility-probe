...
const ROUTE_CONFIG = Object.freeze({
  '0x0063': Object.freeze({
    decision: 'PROMOTE', domain: 'spell_targeter_protocol_event',
    expected_rtti: 'PKT_S2C_StopSpellTargeter_s', callback_owner: 'AIHeroClient',
    hypothesis: 'RTTI_VERIFIED_STOP_SPELL_TARGETER_ROUTE',
    boundary: 'The exact RTTI establishes a stop-spell-targeter protocol event, but the one/two-byte protected field meanings remain unknown.',
  }),
  '0x04a6': Object.freeze({
    decision: 'PROMOTE', domain: 'movement_marker_protocol_event',
    expected_rtti: 'PKT_S2C_MoveMarker_s', callback_owner: 'AIMarker',
    hypothesis: 'RTTI_VERIFIED_MOVE_MARKER_ROUTE',
    boundary: 'The exact RTTI establishes a move-marker route, not coordinates, units, or field-level marker behavior.',
  }),
  '0x02fb': Object.freeze({
    decision: 'PROMOTE', domain: 'inventory_item_modifier_protocol_event',
    expected_rtti: 'PKT_S2C_AddItemModifier_s', callback_owner: 'HeroInventoryClient',
    hypothesis: 'RTTI_VERIFIED_ADD_ITEM_MODIFIER_ROUTE',
    boundary: 'The exact RTTI establishes an add-item-modifier event; item/modifier identifiers and mutation semantics remain unnamed.',
  }),
  '0x0499': Object.freeze({
    decision: 'PROMOTE', domain: 'buff_facing_lock_protocol_event',
    expected_rtti: 'PKT_NPC_BuffLockFacing_s', callback_owner: 'BuffManagerClient',
    hypothesis: 'RTTI_VERIFIED_BUFF_LOCK_FACING_ROUTE',
    boundary: 'The exact RTTI establishes a buff lock-facing event, not the buff identifier, angle encoding, duration, or effect behavior.',
  }),
  '0x030b': Object.freeze({
    decision: 'PROMOTE', domain: 'movement_turn_data_protocol_event',
    expected_rtti: 'PKT_DirectInputMovementDriverServerTurnData_s', callback_owner: 'AIBaseClient',
    hypothesis: 'RTTI_VERIFIED_SERVER_TURN_DATA_ROUTE',
    boundary: 'The exact RTTI establishes server turn data for the movement driver; its protected field dictionary and coordinate conventions remain unknown.',
  }),
  '0x020d': Object.freeze({
    decision: 'PROMOTE', domain: 'spell_numerical_display_protocol_event',
    expected_rtti: 'PKT_S2C_UpdateSpellNumericalDisplay_s', callback_owner: 'AIHeroClient',
    hypothesis: 'RTTI_VERIFIED_UPDATE_SPELL_NUMERICAL_DISPLAY_ROUTE',
    boundary: 'The exact RTTI establishes a spell numerical-display update, not the display slot, numeric value, or UI/gameplay interpretation.',
  }),
  '0x02ed': Object.freeze({
    decision: 'PROMOTE', domain: 'missile_target_change_protocol_event',
    expected_rtti: 'PKT_S2C_ChangeMissileTarget_s', callback_owner: 'MissileClient',
    hypothesis: 'RTTI_VERIFIED_CHANGE_MISSILE_TARGET_ROUTE',
    boundary: 'The exact RTTI establishes a missile-target-change event; source/target field roles and targeting behavior remain unclaimed.',
  }),
  '0x001a': Object.freeze({
    decision: 'PROMOTE', domain: 'unit_info_bar_protocol_event',
    expected_rtti: 'PKT_S2C_UpdateUnitInfoBar_s', callback_owner: 'AIBaseClient',
    hypothesis: 'RTTI_VERIFIED_UPDATE_UNIT_INFO_BAR_ROUTE',
    boundary: 'The exact RTTI establishes a unit-info-bar update; the variable protected payload does not identify health, resource, visibility, or UI fields.',
  }),
  '0x02a6': Object.freeze({
    decision: 'PROMOTE', domain: 'fake_buff_removal_protocol_event',
    expected_rtti: 'PKT_NPC_RemoveFakeBuff_s', callback_owner: 'BuffManagerClient',
    hypothesis: 'RTTI_VERIFIED_REMOVE_FAKE_BUFF_ROUTE',
    boundary: 'The exact RTTI establishes fake-buff removal, but the buff identifier and client-visible effect semantics remain unnamed.',
  }),
});
...
function distinctPayloadRow(event, payload) {
  return {
    schema: 'RESIDUAL_P6_DISTINCT_PAYLOAD_V2',
    schema_version: 2,
...
function buildProfiles(staticRecovery) {
  return ROUTE_IDS.map((packetId) => {
    ...
    return {
      schema: 'RESIDUAL_P6_EXACT_NATIVE_PROFILE_V2',
      schema_version: 2,
      exact_build: EXACT_BUILD,
      exact_runtime_image_sha256: RUNTIME_SHA256,
      static_identity: runtime,
      profile: {
        id: `residual-p6-${packetType.slice(2)}-exact-runtime-v2`,
...
function externalEvidence(runtime, native, config) {
  ...
  pieces.push('a controlled one-property-at-a-time exact-build producer/consumer trace with plaintext field labels');
  return `${pieces.join(', ')}.`;
}
...
function buildDecisions(evidence, staticRecovery, nativeAnalysis) {
  const routeDecisions = ROUTE_IDS.map((packetId) => {
    ...
    const localComplete = runtime.validations.all_factory_checks_pass
      ...
      && runtime.receive_identity.direct_rtti_packet_name_verified
      && runtime.receive_identity.direct_rtti_callback_owner_verified
      && runtime.receive_identity.direct_receive_target_executable_verified
    ...
    return {
      decision_id: `ROUTE_${packetType}_RESIDUAL_P6_WAVE_V2`,
      packet_id: packetId,
      ...
      semantic_claim: `Exact-build callback RTTI identity ${config.expected_rtti} owned by ${config.callback_owner}; protected payload field roles remain unpublished.`,
      structural_claim: `Exact-build ${runtime.factory_chain.object_size}-byte packet object with closed factory/constructor/vtable/deserializer and callback receive identity.`,
      evidence_grade: 'VERIFIED_DIRECT_EXACT_BUILD_RTTI_CALLBACK_STRUCTURE_WITH_FULL_SAFE_INVENTORY',
  ...
  const domainDecisions = [...new Set(routeDecisions.map((row) => row.domain))].map((domain) => {
    ...
    const decision = members.every((row) => row.decision === 'PROMOTE') ? 'PROMOTE' : 'KEEP_CANDIDATE';
    return {
      decision_id: `DOMAIN_${domain.toUpperCase()}_RESIDUAL_P6_V2`,
      domain,
      routes: members.map((row) => row.packet_discriminator),
      capabilities: members.map((row) => row.capability),
      decision,
      semantic_claim: `Exact RTTI-named protocol domain; field-level semantics remain external-state gated.`,
      actual_reverse_engineering_executed: true,
...
function validateDecisionBundle(bundle) {
  invariant(bundle.schema === 'RESIDUAL_P6_WAVE_MACHINE_DECISIONS_V2', 'decision schema mismatch');
...
function markdownReport(report) {
  const lines = [
    '# Residual P6 route wave — exact-build deep recovery audit',
    '',
  ...
  lines.push(
    '',
    '## Promotion boundary and counterexamples',
    '',
    'All nine routes have unique exact-build callback RTTI names and receive-owner targets. Promotion is limited to those protocol identities. Native storage lanes, timing, adjacency, entity candidates, and anchor correlations do not name protected fields or prove gameplay behavior.',
    '',
    '`0x001a` uses a fragmented `.pdata` constructor range. The factory-proven entry was decoded through its first real `ret`, directly recovering the `0x001a` ID write, 104-byte object vtable, and deserializer; the truncated exception-directory range is recorded as a tooling counterexample.',
    '',
    '## Saturation boundary',
    '',
    'Every authorized latest-four occurrence, distinct payload, exact factory/constructor/vtable/deserializer chain, static callback/RTTI surface, full PE function-range packet-ID immediate surface, native branch, stream/entity/time/cross-route relationship, and requested anchor/control was executed. Remaining protected field names and behavioral meanings require controlled exact-build producer/consumer state evidence.',
    '',
  );
  return lines.join('\n');
}
