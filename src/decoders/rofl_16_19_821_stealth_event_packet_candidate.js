...
const CAPABILITY = 'stealth_event_packet';
const PACKET_ID = 0x040a;
const PAYLOAD_LENGTH = 17;
const EVENT_NAMES = new Map([[0x0101, 'OnEnterStealth'], [0x0102, 'OnExitStealth']]);
const CONTROL_IDS = new Set([0x00cb, 0x00cf, 0x0107, 0x0113, 0x0114, 0x0115]);
const MAX_IMAGE_BYTES = 64 * 1024 * 1024;
const MAX_TOTAL_PACKETS = 5_000;
const MAX_BATCH_PACKETS = 1_500;
const MAX_REQUEST_BYTES = 1_000_000;

const STEALTH_EVENT_PACKET_CANDIDATE_PROFILE_821 = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-stealth-event-packet-runtime-candidate-v1',
  ...
  child_event_ids: Object.freeze([...EVENT_NAMES.keys()]),
  registered_event_names: Object.freeze([...EVENT_NAMES.values()]),
  evidence_runtime_image_sha256: IMAGE_SHA256,
  evidence_packet_callback_rva: '0x4ce3a0',
  evidence_child_registration_rvas: Object.freeze(['0x2722f3', '0x272310']),
  evidence_child_callback_rvas: Object.freeze(['0x2b9e80', '0x2ba0a0']),
  evidence_event_name_table_rva: '0x1ef7330',
  runtime_image_required: true,
  evidence_scope: 'exact 821 image registration and name table identify children 0x0101/0x0102; native deserializer fully consumes all 5621 observed length-17 OnEvent packets across 11 KR Replays, including 708 same-length control children',
  known_limits: Object.freeze([
    'The names are exact-image event labels, not proof of a visibility effect or persistent stealth state.',
    'Child +0x04 is callback-read for an object lookup; its object role and relation to Replay raw_param are unassigned.',
    'Replay raw_param and child +0x04 differ in 72 of 4913 target packets and are both preserved.',
    'Only observed 17-byte game-stream OnEvent packets are accepted; native child ID distinguishes target from six same-length controls.',
    'Unobserved same-length child IDs and changed child shapes fail closed.',
...
function decodeStealthEventPacketCandidates821(replay, {
  runtimeImagePath, pythonExecutable, precollected,
} = {}) {
  const profile = STEALTH_EVENT_PACKET_CANDIDATE_PROFILE_821;
  const base = {
    profile_id: profile.id,
    input_packet_id: PACKET_ID,
    input_packet_scope: 'child_0101_0102_length_17',
    child_event_ids: [...EVENT_NAMES.keys()],
    evidence_runtime_image_sha256: IMAGE_SHA256,
  };
  ...
  if (replay?.header?.version !== REPLAY_VERSION) {
    return fail('UNSUPPORTED', `stealth-event packet candidate supports only ${REPLAY_VERSION}`);
  }
  ...
  if (observedPacketCount > MAX_TOTAL_PACKETS) {
    return fail('UNSUPPORTED', `stealth-event runtime input exceeds ${MAX_TOTAL_PACKETS} packets`, {
      observed_packet_count_minimum: observedPacketCount,
  ...
  if (!Array.isArray(rows)) {
    return fail('DECODE_FAILED', '821 stealth-event route scan returned no packet rows', {
      scanned_block_count: scannedBlockCount,
    });
  }
  if (rows.length === 0) {
    return fail('PROFILE_UNAVAILABLE', 'KR 0x040a length-17 packet shape is absent', {
  ...
  for (const { block, chunk } of rows) {
    if (chunk.stream_tag !== 1 || chunk.stream !== 'game_chunk'
      ...
      return failed('DECODE_FAILED', '0x040a length-17 packet framing differs from observed KR scope', {
  ...
  const script = path.resolve(__dirname, '..', '..', 'scripts',
    'decode_stealth_event_packet_16_19_821.py');
  const events = [];
  ...
  for (let start = 0; start < rows.length; start += MAX_BATCH_PACKETS) {
    ...
    if (Buffer.byteLength(request) > MAX_REQUEST_BYTES) {
      return failed('UNSUPPORTED', 'stealth-event runtime request exceeds bounded input size', {
    ...
    if (run.error || run.status !== 0) {
      ...
      return failed(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `exact runtime stealth-event packet decoder failed: ${detail}`, {
    ...
    try {
      ...
      return failed('DECODE_FAILED', `runtime stealth-event output is not JSON: ${error.message}`, {
    ...
    if (decoded?.status !== 'PASS' || decoded.runtime_image_sha256 !== IMAGE_SHA256
      ...
      return failed('DECODE_FAILED', 'runtime stealth-event output identity or packet count differs', {
        runtime_image_used: start > 0, runtime_image_status: 'EXECUTION_FAILED',
      });
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      const blob = /^[0-9a-f]{16}$/.test(row?.event_blob_hex ?? '')
        ? Buffer.from(row.event_blob_hex, 'hex') : null;
      const isTarget = EVENT_NAMES.has(row?.event_id);
      const expectedRawId = row?.event_id === 0x0101 ? '0x0909' : '0x0929';
      if (!row || typeof row !== 'object'
          || (row.status !== 'DECODED' && row.status !== 'EXCLUDED_CHILD')
          ...
          || (isTarget && row.raw_event_id_hex !== expectedRawId)
          || row.event_blob_length !== 8 || blob === null
          ...
          || (isTarget && row.event_schema_u32_0x00 !== 469)) {
        return failed('DECODE_FAILED', `runtime stealth-event packet ${start + index} did not fully decode`, {
      ...
      events.push({
        event_type: 'STEALTH_EVENT_PACKET_CANDIDATE',
        ...
        registered_event_name: EVENT_NAMES.get(row.event_id),
...
module.exports = {
  STEALTH_EVENT_PACKET_CANDIDATE_PROFILE_821,
  decodeStealthEventPacketCandidates821,
};
