...
const CALLBACK_TABLE_SHA256 = Object.freeze({
  opaque_u8_0x10: '5ab1986ede229081b774b678b55389a3f0984de31e969f04b9745cd41de24d0a',
  opaque_u32_0x14: '0a2ced1eb36b6eb3436e42dc4a057436c7bc01a28cf72e6b98e631a97910fb27',
  opaque_u8_0x18: '61fc4c46a9d37a0903bbf158b451700ef9fe3a1e696c03e64ed6ff822c29a0a6',
  opaque_u32_0x1c: '3340642441bdc1f66c2c855975ae3e284b950a52c5dc07ff2a065bf3888a6e0c',
});
const PACKET_ID = 0x0194;
const CAPABILITY = 'npc_buff_update_num_counter_packet';
const MAX_IMAGE_BYTES = 64 * 1024 * 1024;
const MAX_BATCH_PACKETS = 20_000;
const MAX_TOTAL_PACKETS = 25_000;
const MAX_REQUEST_BYTES = 4_000_000;
const OBSERVED_PAYLOAD_LENGTHS = new Set([6, 7, 8, 9]);

const NPC_BUFF_UPDATE_NUM_COUNTER_PACKET_CANDIDATE_PROFILE_821 = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-buff-update-num-counter-packet-runtime-candidate-v1',
  ...
  packet_name: 'PKT_NPC_BuffUpdateNumCounter_s',
  evidence_runtime_image_sha256: IMAGE_SHA256,
  evidence_callback_table_sha256: CALLBACK_TABLE_SHA256,
  runtime_image_required: true,
  evidence_scope: 'exact 821 registration/callback; 18734/18734 and 13620/13620 native full packets in two KR Replays; 44/44 route-shape samples from 11 KR Replays',
  known_limits: Object.freeze([
    'The packet class name and decoded fields do not prove a successful buff counter update.',
    'Object offsets 0x10, 0x14, 0x18 and 0x1c remain anonymous; no owner, buff identity, target, counter meaning or lifecycle is inferred.',
...
function decodeNpcBuffUpdateNumCounterPacketCandidates821(replay, {
  runtimeImagePath, pythonExecutable, precollected,
} = {}) {
  const profile = NPC_BUFF_UPDATE_NUM_COUNTER_PACKET_CANDIDATE_PROFILE_821;
  const base = {
  ...
  if (replay?.header?.version !== REPLAY_VERSION) {
    return fail('UNSUPPORTED', `BuffUpdateNumCounter packet candidate supports only ${REPLAY_VERSION}`);
  }
  ...
  if (selected.observed_packet_count_minimum > MAX_TOTAL_PACKETS) {
    return fail('UNSUPPORTED', `BuffUpdateNumCounter runtime input exceeds ${MAX_TOTAL_PACKETS} packets`, {
      observed_packet_count_minimum: selected.observed_packet_count_minimum,
      scanned_block_count: scannedBlockCount,
    });
  }
  if (rows.length === 0) {
    return fail('PROFILE_UNAVAILABLE', 'KR 0x0194 BuffUpdateNumCounter route is absent', {
  ...
  if (inputCount > MAX_TOTAL_PACKETS) {
    return failed('UNSUPPORTED', `BuffUpdateNumCounter runtime input exceeds ${MAX_TOTAL_PACKETS} packets`);
  }
  for (const { block, chunk } of rows) {
    ...
    if (chunk.stream_tag !== 1 || block.packet_id !== PACKET_ID
      ...
      return failed('DECODE_FAILED', '0x0194 packet framing differs from observed KR scope', {
        first_failed_packet_ref: packetRef(replay, block, chunk),
      });
    }
    if (!OBSERVED_PAYLOAD_LENGTHS.has(block.payload_length)) {
      return failed('DECODE_FAILED', '0x0194 payload length differs from observed KR scope', {
        first_failed_packet_ref: packetRef(replay, block, chunk),
      });
    }
  }
  ...
  const script = path.resolve(__dirname, '..', '..', 'scripts',
    'decode_buff_update_num_counter_packet_16_19_821.py');
  const events = [];
  for (let start = 0; start < rows.length; start += MAX_BATCH_PACKETS) {
    ...
    if (Buffer.byteLength(request) > MAX_REQUEST_BYTES) {
      return failed('UNSUPPORTED', 'BuffUpdateNumCounter runtime request exceeds bounded input size', {
    ...
    if (run.error || run.status !== 0) {
      ...
      return failed(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `exact runtime BuffUpdateNumCounter packet decoder failed: ${detail}`, {
    ...
    try {
      ...
      return failed('DECODE_FAILED', `runtime BuffUpdateNumCounter packet output is not JSON: ${error.message}`, {
    ...
    if (decoded?.status !== 'PASS' || decoded.runtime_image_sha256 !== IMAGE_SHA256
      ...
      return failed('DECODE_FAILED', 'runtime BuffUpdateNumCounter output identity or packet count differs', {
        runtime_image_used: start > 0, runtime_image_status: 'EXECUTION_FAILED',
      });
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      if (row?.status !== 'DECODED' || row.input_index !== index
          ...
          || !Number.isInteger(row.opaque_u8_0x18) || row.opaque_u8_0x18 < 0
          || row.opaque_u8_0x18 > 0xff
          ...
          || !/^[0-9a-f]{2}$/.test(row.raw_u8_0x18_hex)
          || !/^[0-9a-f]{8}$/.test(row.raw_u32_0x1c_hex)) {
        return failed('DECODE_FAILED', `runtime BuffUpdateNumCounter packet ${start + index} did not fully decode`, {
      ...
      events.push({
        event_type: 'NPC_BUFF_UPDATE_NUM_COUNTER_PACKET_CANDIDATE',
        ...
        opaque_u8_0x18: row.opaque_u8_0x18,
        ...
        raw_object_u8_0x18_hex: row.raw_u8_0x18_hex,
  ...
  return {
    ...
    event_field_confidence: {
      ...
      opaque_u8_0x18: 'CANDIDATE_EXACT_RUNTIME_FIELD',
...
module.exports = {
  NPC_BUFF_UPDATE_NUM_COUNTER_PACKET_CANDIDATE_PROFILE_821,
  decodeNpcBuffUpdateNumCounterPacketCandidates821,
};
