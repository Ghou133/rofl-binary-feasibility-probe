...
const CAPABILITY = 'dampener_die_event_packet';
const PACKET_ID = 0x040a;
const CHILD_EVENT_ID = 0x0035;
const RAW_EVENT_ID_HEX = '0x4906';
...
const DAMPENER_DIE_EVENT_PACKET_821_PROFILE = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-on-dampener-die-event-packet-runtime-candidate-v1',
  ...
  child_event_name: 'OnDampenerDie',
  evidence_runtime_image_sha256: IMAGE_SHA256,
  evidence_event_name_entry_rva: '0x1ef7b78',
  runtime_image_required: true,
  evidence_scope: '11 exact-build KR Replays: 18 native child 0x0035 packets and 813 same-length foreign-child controls among 831 fully consumed 0x040a/116 parents',
  known_limits: Object.freeze([
    'The event name is an exact-image label. A packet does not prove a dampener death, actor role, or game-state transition.',
    'The 108-byte native child blob is anonymous; its field roles and any effect are UNKNOWN.',
    'The 116-byte parent route has four observed same-length foreign children. Raw suffix fingerprints only select native checks; the native child ID is authoritative.',
    'The 18 target packets occur across 10 of the 11 supplied KR Replays. Absence in another Replay is PROFILE_UNAVAILABLE, not an observed zero count.',
...
function rawShape(payload) {
  ...
  if (tail === '247152269506'
      && payload.subarray(1, 5).equals(Buffer.from('1eb9b33d', 'hex'))) return 'target';
  if (tail === '247152269548') return 'observed_foreign_0004';
  if (tail === '247152269566') return 'observed_foreign_003b';
  if (tail === '247152269586') return 'observed_foreign_003d';
  if (tail === '247152269518') return 'observed_foreign_0046';
  return null;
}

function decodeDampenerDieEventPacketCandidates821(replay, {
  runtimeImagePath, pythonExecutable, precollected,
} = {}) {
  const profile = DAMPENER_DIE_EVENT_PACKET_821_PROFILE;
  const base = {
    profile_id: profile.id,
    input_packet_id: PACKET_ID,
    input_packet_scope: 'child_0035_length_116',
    child_event_id: CHILD_EVENT_ID,
    evidence_runtime_image_sha256: IMAGE_SHA256,
  };
  ...
  if (replay?.header?.version !== REPLAY_VERSION) {
    return fail('UNSUPPORTED', `OnDampenerDie packet candidate supports only ${REPLAY_VERSION}`);
  }
  ...
  if (observedPacketCount > MAX_TOTAL_PACKETS) {
    return fail('UNSUPPORTED', `OnDampenerDie runtime input exceeds ${MAX_TOTAL_PACKETS} packets`, {
      observed_packet_count_minimum: observedPacketCount,
  ...
  if (!Array.isArray(rows)) {
    return fail('DECODE_FAILED', '821 OnDampenerDie route scan returned no packet rows', {
      scanned_block_count: scannedBlockCount,
    });
  }
  const target = [];
  ...
  if (target.length === 0) {
    return fail('PROFILE_UNAVAILABLE', 'KR 0x040a/116 OnDampenerDie raw shape is absent', {
  ...
  const script = path.resolve(__dirname, '..', '..', 'scripts',
    'decode_dampener_die_event_packet_16_19_821.py');
  const events = [];
  for (let start = 0; start < target.length; start += MAX_BATCH_PACKETS) {
    ...
    if (Buffer.byteLength(request) > MAX_REQUEST_BYTES) {
      return failed('UNSUPPORTED', 'OnDampenerDie runtime request exceeds bounded input size', {
    ...
    if (run.error || run.status !== 0) {
      ...
      return failed(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `exact runtime OnDampenerDie packet decoder failed: ${detail}`, {
    ...
    try {
      ...
      return failed('DECODE_FAILED', `runtime OnDampenerDie packet output is not JSON: ${error.message}`, {
    ...
    if (decoded?.status !== 'PASS' || decoded.runtime_image_sha256 !== IMAGE_SHA256
      ...
      return failed('DECODE_FAILED', 'runtime OnDampenerDie output identity or packet count differs', {
        runtime_image_used: start > 0, runtime_image_status: 'EXECUTION_FAILED',
      });
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      if (!valid || sha256(blob) !== row.event_blob_sha256) {
        return failed('DECODE_FAILED', `runtime OnDampenerDie packet ${start + index} did not match exact child scope`, {
...
module.exports = {
  DAMPENER_DIE_EVENT_PACKET_821_PROFILE,
  decodeDampenerDieEventPacketCandidates821,
};
