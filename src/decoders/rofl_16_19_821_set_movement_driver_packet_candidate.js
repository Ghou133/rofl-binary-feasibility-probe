...
const CALLBACK_TRANSFORM_SHA256 = '49a25c8d31448ba9cbcc1810c937e3c297a6a85e0724436994e8d31501cd8526';
const PACKET_ID = 0x0335;
const CAPABILITY = 'set_movement_driver_packet';
const MAX_IMAGE_BYTES = 64 * 1024 * 1024;
const MAX_BATCH_PACKETS = 8192;
const MAX_TOTAL_PACKETS = 20_000;
const MAX_REQUEST_BYTES = 4_000_000;
const OBSERVED_SHAPES = Object.freeze({
  '2:84': Object.freeze({ raw: '09', transformed: 1 }),
  '28:38': Object.freeze({ raw: '29', transformed: 2 }),
});

const SET_MOVEMENT_DRIVER_PACKET_CANDIDATE_PROFILE_821 = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-set-movement-driver-packet-runtime-candidate-v1',
  ...
  packet_name: 'PKT_S2C_SetMovementDriver_s',
  evidence_runtime_image_sha256: IMAGE_SHA256,
  evidence_callback_transform_sha256: CALLBACK_TRANSFORM_SHA256,
  runtime_image_required: true,
  evidence_scope: 'exact 821 registration, constructor, deserializer and callback; 131/131 route packets fully consumed across 11 KR Replays',
  known_limits: Object.freeze([
    'The transformed object byte is passed as a callback dispatch selector; it does not prove a successful driver change.',
    'The packet does not establish path, position, movement action, participant, owner or target.',
    'Native object +0x0c is populated by the Replay-param base-reader hook; agreement is only a consistency check.',
    'Only the two observed game-stream shapes are accepted. A zeroed first byte can still be fully consumed natively.',
...
function decodeSetMovementDriverPacketCandidates821(replay, {
  runtimeImagePath, pythonExecutable, precollected,
} = {}) {
  const profile = SET_MOVEMENT_DRIVER_PACKET_CANDIDATE_PROFILE_821;
  const base = {
  ...
  if (replay?.header?.version !== REPLAY_VERSION) {
    return fail('UNSUPPORTED', `SetMovementDriver packet candidate supports only ${REPLAY_VERSION}`);
  }
  ...
  if (selected.observed_packet_count_minimum > MAX_TOTAL_PACKETS) {
    return fail('UNSUPPORTED', `SetMovementDriver runtime input exceeds ${MAX_TOTAL_PACKETS} packets`, {
      observed_packet_count_minimum: selected.observed_packet_count_minimum,
      scanned_block_count: scannedBlockCount,
    });
  }
  if (rows.length === 0) {
    return fail('PROFILE_UNAVAILABLE', 'KR 0x0335 SetMovementDriver route is absent', {
  ...
  if (inputCount > MAX_TOTAL_PACKETS) {
    return failed('UNSUPPORTED', `SetMovementDriver runtime input exceeds ${MAX_TOTAL_PACKETS} packets`);
  }
  for (const { block, chunk } of rows) {
    ...
    if (chunk.stream_tag !== 1 || block.packet_id !== PACKET_ID
        ...
        || !OBSERVED_SHAPES[`${block.payload_length}:${block.payload[0]}`]) {
      return failed('DECODE_FAILED', '0x0335 packet differs from observed KR stream, selector or length', {
  ...
  const script = path.resolve(__dirname, '..', '..', 'scripts',
    'decode_set_movement_driver_packet_16_19_821.py');
  const events = [];
  for (let start = 0; start < rows.length; start += MAX_BATCH_PACKETS) {
    ...
    if (Buffer.byteLength(request) > MAX_REQUEST_BYTES) {
      return failed('UNSUPPORTED', 'SetMovementDriver runtime request exceeds bounded input size', {
    ...
    if (run.error || run.status !== 0) {
      ...
      return failed(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `exact runtime SetMovementDriver decoder failed: ${detail}`, {
    ...
    try {
      ...
      return failed('DECODE_FAILED', `runtime SetMovementDriver output is not JSON: ${error.message}`, {
    ...
    if (decoded?.status !== 'PASS' || decoded.runtime_image_sha256 !== IMAGE_SHA256
      ...
      return failed('DECODE_FAILED', 'runtime SetMovementDriver output identity or packet count differs', {
        runtime_image_used: start > 0 ? true : null,
        runtime_image_status: 'EXECUTION_FAILED',
      });
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      const expected = OBSERVED_SHAPES[`${block.payload_length}:${block.payload[0]}`];
      if (row?.status !== 'DECODED' || row.input_index !== index
          ...
          || row.raw_u8_byte_0x2a_hex !== expected.raw
          || row.opaque_u8_0x2a !== expected.transformed) {
        return failed('DECODE_FAILED', `runtime SetMovementDriver packet ${start + index} did not fully decode`, {
      ...
      events.push({
        event_type: 'SET_MOVEMENT_DRIVER_PACKET_CANDIDATE',
        ...
        raw_payload_byte_0: block.payload[0],
        raw_object_u8_byte_0x2a_hex: row.raw_u8_byte_0x2a_hex,
        opaque_u8_0x2a: row.opaque_u8_0x2a,
        confidence: 'CANDIDATE',
        semantic_status: 'CANDIDATE_EXACT_RUNTIME_PACKET_FIELD',
        raw_packet_ref: ref,
      });
    }
  }
  return {
    ...
    evidence_status: 'CANDIDATE_EXACT_RUNTIME_PACKET_FIELD',
    known_limits: [...profile.known_limits],
    event_field_confidence: {
      replay_time_ms: 'VERIFIED_DIRECT',
      raw_param: 'VERIFIED_DIRECT',
      raw_payload_byte_0: 'VERIFIED_DIRECT',
      opaque_u8_0x2a: 'CANDIDATE_EXACT_RUNTIME_FIELD',
    },
