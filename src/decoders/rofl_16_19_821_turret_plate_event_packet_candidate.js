...
const CAPABILITY = 'turret_plate_event_packet';
const PACKET_ID = 0x040a;
const CHILD_EVENT_ID = 0x0107;
const RAW_EVENT_ID_HEX = '0x09e8';
const CONTROL_IDS = new Set([0x00cb, 0x00cf, 0x0101, 0x0102, 0x0113, 0x0114, 0x0115]);
...
const TURRET_PLATE_EVENT_PACKET_821_PROFILE = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-turret-plate-event-packet-runtime-candidate-v1',
  ...
  child_event_name: 'OnTurretPlateDestroyed',
  evidence_runtime_image_sha256: IMAGE_SHA256,
  evidence_event_name_table_rva: '0x1ef7330',
  evidence_event_name_entry_rva: '0x1ef9c48',
  runtime_image_required: true,
  evidence_scope: 'exact 821 image name table labels child 0x0107 OnTurretPlateDestroyed; native parent fully consumes all 5621 observed length-17 packets, including 657 target and 4964 same-length foreign controls across 11 KR Replays',
  known_limits: Object.freeze([
    'The event name is an exact-image table label. A packet does not prove a turret plate was destroyed or any game-state transition.',
    'Native child +0x04 is an anonymous u32 value. Actor, target, object, and effect roles are UNKNOWN.',
    'No child-specific callback or callback-read field mapping has been established for 0x0107.',
    'The 657 target packets occur in all 11 KR Replays; 4964 native same-length foreign-child packets are explicitly excluded.',
    'Replay raw_param differs from native child +0x04 in all 657 targets and is preserved separately.',
    'The observed child +0x00 value 469 and raw event ID 0x09e8 are exact-sample gates. Other shapes fail closed.',
...
function decodeTurretPlateEventPacketCandidates821(replay, {
  runtimeImagePath, pythonExecutable, precollected,
} = {}) {
  const profile = TURRET_PLATE_EVENT_PACKET_821_PROFILE;
  const base = {
    profile_id: profile.id,
    input_packet_id: PACKET_ID,
    input_packet_scope: 'child_0107_length_17',
    child_event_id: CHILD_EVENT_ID,
    evidence_runtime_image_sha256: IMAGE_SHA256,
  };
  ...
  if (replay?.header?.version !== REPLAY_VERSION) {
    return fail('UNSUPPORTED', `OnTurretPlateDestroyed packet candidate supports only ${REPLAY_VERSION}`);
  }
  ...
  if (observedPacketCount > MAX_TOTAL_PACKETS) {
    return fail('UNSUPPORTED', `OnTurretPlateDestroyed runtime input exceeds ${MAX_TOTAL_PACKETS} packets`, {
      observed_packet_count_minimum: observedPacketCount,
  ...
  if (!Array.isArray(rows)) {
    return fail('DECODE_FAILED', '821 OnTurretPlateDestroyed route scan returned no packet rows', {
      scanned_block_count: scannedBlockCount,
    });
  }
  if (rows.length === 0) {
  ...
  for (const { block, chunk } of rows) {
    if (chunk.stream_tag !== 1 || chunk.stream !== 'game_chunk'
      ...
      return failed('DECODE_FAILED', '0x040a OnTurretPlateDestroyed packet framing differs from observed KR scope', {
  ...
  const script = path.resolve(__dirname, '..', '..', 'scripts',
    'decode_turret_plate_event_packet_16_19_821.py');
  const events = [];
  ...
  for (let start = 0; start < rows.length; start += MAX_BATCH_PACKETS) {
    ...
    if (Buffer.byteLength(request) > MAX_REQUEST_BYTES) {
      return failed('UNSUPPORTED', 'OnTurretPlateDestroyed runtime request exceeds bounded input size', {
    ...
    if (run.error || run.status !== 0) {
      ...
      return failed(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `exact runtime OnTurretPlateDestroyed packet decoder failed: ${detail}`, {
    ...
    try {
      ...
      return failed('DECODE_FAILED', `runtime OnTurretPlateDestroyed packet output is not JSON: ${error.message}`, {
    ...
    if (decoded?.status !== 'PASS' || decoded.runtime_image_sha256 !== IMAGE_SHA256
      ...
      return failed('DECODE_FAILED', 'runtime OnTurretPlateDestroyed output identity or packet count differs', {
        runtime_image_used: start > 0, runtime_image_status: 'EXECUTION_FAILED',
      });
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      if (!valid || sha256(blob) !== row.event_blob_sha256
        ...
        return failed('DECODE_FAILED', `runtime OnTurretPlateDestroyed packet ${start + index} did not match exact child scope`, {
      ...
      events.push({
        event_type: 'TURRET_PLATE_EVENT_PACKET_CANDIDATE',
  ...
  if (events.length === 0) {
    return failed('PROFILE_UNAVAILABLE', 'OnEvent child 0x0107 was not observed among native length-17 packets', {
...
module.exports = {
  TURRET_PLATE_EVENT_PACKET_821_PROFILE,
  decodeTurretPlateEventPacketCandidates821,
};
