...
const CAPABILITY = 'change_missile_target_packet';
const PACKET_ID = 0x040c;
...
const OBSERVED_SHORT_PREFIXES = new Set([0x91, 0x93, 0x95, 0x99, 0x9d, 0x9f]);
...
// Exact 821 callback RVA 0x997ca1..0x997cc3 transforms object +0x1c before
// comparing it with a live receiver-dependent value at +0xa0.
function decodeProtectedChangeMissileTargetComparisonKeyU32(rawHex) {
...
function isObservedChangeMissileTargetPayload(payload) {
  return Buffer.isBuffer(payload)
    && ((payload.length === 13 && payload[0] === 0x96)
      || ((payload.length === 3 || payload.length === 4)
        && OBSERVED_SHORT_PREFIXES.has(payload[0])));
}

const CHANGE_MISSILE_TARGET_PACKET_CANDIDATE_PROFILE_821 = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-change-missile-target-packet-candidate-v1',
  ...
  packet_name: 'PKT_S2C_ChangeMissileTarget_s',
  evidence_runtime_image_sha256: IMAGE_SHA256,
  evidence_factory_case_rva: '0xf0b2cf',
  evidence_constructor_rva: '0xea4f10',
  evidence_deserializer_rva: '0x10e8e30',
  evidence_callback_rva: '0x997c80',
  evidence_callback_pre_compare_rva: '0x997cd0',
  runtime_image_required: true,
  evidence_scope: 'Seven of 11 exact-build KR Replays have 13,125 game 0x040c packets across 13 observed length/prefix shapes; all fully consumed natively. The exact callback exposes packet-local +0x1c comparison u32 before consulting receiver state.',
...
function decodeChangeMissileTargetPacketCandidates821(replay, {
  ...
  if (replay?.header?.version !== BUILD) {
    return fail('UNSUPPORTED', `ChangeMissileTarget packet candidate supports only ${BUILD}`);
  }
  ...
  if (observedCount > MAX_PACKETS) {
    return fail('UNSUPPORTED', `change-missile-target input exceeds ${MAX_PACKETS} packets`, {
      input_count: observedCount, scanned_block_count: scannedBlockCount,
    });
  }
  if (!Array.isArray(rows) || rows.length !== observedCount) {
    return fail('DECODE_FAILED', 'change-missile-target route scan returned incomplete rows', {
      input_count: observedCount, scanned_block_count: scannedBlockCount,
    });
  }
  if (!rows.length) return fail('PROFILE_UNAVAILABLE', 'KR 0x040c packet route is absent', {
  ...
  for (const { block, chunk } of rows) {
    if (!block || !chunk || block.packet_id !== PACKET_ID
        ...
        || !isObservedChangeMissileTargetPayload(block.payload)
      ...
      return fail('DECODE_FAILED', '0x040c packet has unobserved length or invalid source', {
        input_count: observedCount, scanned_block_count: scannedBlockCount,
  ...
  const script = path.resolve(__dirname, '..', '..', 'scripts',
    'decode_change_missile_target_packet_16_19_821.py');
  ...
  for (let start = 0; start < rows.length; start += MAX_NATIVE_BATCH) {
    ...
    if (run.error || run.status !== 0) {
      ...
      return fail(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `change-missile-target native witness failed: ${detail}`, {
          input_count: observedCount, scanned_block_count: scannedBlockCount,
