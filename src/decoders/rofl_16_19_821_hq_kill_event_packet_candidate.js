...
const CAPABILITY = 'hq_kill_event_packet';
const PACKET_ID = 0x040a;
const CHILD_EVENT_ID = 0x0046;
const RAW_EVENT_ID_HEX = '0x4918';
...
const HQ_KILL_EVENT_PACKET_821_PROFILE = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-on-hq-kill-event-packet-runtime-candidate-v1',
  ...
  child_event_name: 'OnHQKill',
  evidence_runtime_image_sha256: IMAGE_SHA256,
  evidence_event_name_entry_rva: '0x1ef7e20',
  runtime_image_required: true,
  evidence_scope: '11 exact-build KR Replays: 11 native child 0x0046 packets and 820 same-length foreign-child controls among 831 fully consumed 0x040a/116 parents',
  known_limits: Object.freeze([
    'The event name is an exact-image label. A packet does not prove HQ destruction, a winner, actor role, or game-state transition.',
...
function rawShape(payload) {
  ...
  if (tail === '247152269518') return 'target';
...
function decodeHqKillEventPacketCandidates821(replay, {
  runtimeImagePath, pythonExecutable, precollected,
} = {}) {
  const profile = HQ_KILL_EVENT_PACKET_821_PROFILE;
  const base = {
    profile_id: profile.id,
    input_packet_id: PACKET_ID,
    input_packet_scope: 'child_0046_length_116',
    child_event_id: CHILD_EVENT_ID,
    evidence_runtime_image_sha256: IMAGE_SHA256,
  };
  ...
  if (replay?.header?.version !== REPLAY_VERSION) {
    return fail('UNSUPPORTED', `OnHQKill packet candidate supports only ${REPLAY_VERSION}`);
  }
  ...
  if (observedPacketCount > MAX_TOTAL_PACKETS) {
    return fail('UNSUPPORTED', `OnHQKill runtime input exceeds ${MAX_TOTAL_PACKETS} packets`, {
      observed_packet_count_minimum: observedPacketCount,
  ...
  if (!Array.isArray(rows)) {
    return fail('DECODE_FAILED', '821 OnHQKill route scan returned no packet rows', {
      scanned_block_count: scannedBlockCount,
    });
  }
  const target = [];
  ...
  if (target.length === 0) {
    return fail('PROFILE_UNAVAILABLE', 'KR 0x040a/116 OnHQKill raw shape is absent', {
  ...
  const script = path.resolve(__dirname, '..', '..', 'scripts',
    'decode_hq_kill_event_packet_16_19_821.py');
  const events = [];
  for (let start = 0; start < target.length; start += MAX_BATCH_PACKETS) {
    ...
    if (Buffer.byteLength(request) > MAX_REQUEST_BYTES) {
      return failed('UNSUPPORTED', 'OnHQKill runtime request exceeds bounded input size', {
    ...
    if (run.error || run.status !== 0) {
      ...
      return failed(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `exact runtime OnHQKill packet decoder failed: ${detail}`, {
    ...
    try {
      ...
      return failed('DECODE_FAILED', `runtime OnHQKill packet output is not JSON: ${error.message}`, {
    ...
    if (decoded?.status !== 'PASS' || decoded.runtime_image_sha256 !== IMAGE_SHA256
      ...
      return failed('DECODE_FAILED', 'runtime OnHQKill output identity or packet count differs', {
        runtime_image_used: start > 0, runtime_image_status: 'EXECUTION_FAILED',
      });
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      if (!valid || sha256(blob) !== row.event_blob_sha256) {
        return failed('DECODE_FAILED', `runtime OnHQKill packet ${start + index} did not match exact child scope`, {
...
module.exports = {
  HQ_KILL_EVENT_PACKET_821_PROFILE,
  decodeHqKillEventPacketCandidates821,
};
