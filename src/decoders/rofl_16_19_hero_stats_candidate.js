...
const KILL_STATS_FIELDS = Object.freeze([
  ...
  Object.freeze({ tailField: 'LARGEST_MULTI_KILL', candidateKey: 'largest_multi_kill_candidate', offset: 0x60 }),
  Object.freeze({ tailField: 'DOUBLE_KILLS', candidateKey: 'double_kills_candidate', offset: 0x64 }),
...
const DAMAGE_TOTAL_FIELDS = Object.freeze([
  Object.freeze({ tailField: 'TOTAL_DAMAGE_DEALT_TO_CHAMPIONS', candidateKey: 'damage_to_champions_raw_f32_candidate', floorKey: 'damage_to_champions_floor_candidate', offset: 0x1e0 }),
  Object.freeze({ tailField: 'TOTAL_DAMAGE_DEALT', candidateKey: 'total_damage_dealt_raw_f32_candidate', floorKey: 'total_damage_dealt_floor_candidate', offset: 0x1d0 }),
  Object.freeze({ tailField: 'TOTAL_DAMAGE_TAKEN', candidateKey: 'total_damage_taken_raw_f32_candidate', floorKey: 'total_damage_taken_floor_candidate', offset: 0x1f0 }),
]);
...
const HERO_KILL_STATS_SNAPSHOT_CANDIDATE_PROFILE = Object.freeze({
  id: 'rofl-16.19.820.7193-hn-hero-kill-stats-keyframe-candidate-v1',
  replay_version: REPLAY_VERSION,
  ...
  payload_length: PAYLOAD_LENGTH,
  decoded_blob_length: BLOB_LENGTH,
  largest_killing_spree_u32le_offset_candidate: 0x58,
  killing_sprees_u32le_offset_candidate: 0x5c,
  largest_multi_kill_u32le_offset_candidate: 0x60,
  double_kills_u32le_offset_candidate: 0x64,
  triple_kills_u32le_offset_candidate: 0x68,
...
const HERO_DAMAGE_TOTALS_SNAPSHOT_CANDIDATE_PROFILE = Object.freeze({
  id: 'rofl-16.19.820.7193-hn-hero-damage-totals-keyframe-candidate-v1',
  replay_version: REPLAY_VERSION,
  ...
  payload_length: PAYLOAD_LENGTH,
  decoded_blob_length: BLOB_LENGTH,
  damage_to_champions_f32le_offset_candidate: 0x1e0,
  total_damage_dealt_f32le_offset_candidate: 0x1d0,
  total_damage_taken_f32le_offset_candidate: 0x1f0,
  evidence_runtime_image_sha256: RUNTIME_IMAGE_SHA256,
  lookup_table_sha256: LOOKUP_TABLE_SHA256,
  evidence_scope: 'exact HN HeroStats route and transform; three f32 offsets correlate after flooring with Replay-tail damage totals across 350 participant snapshots from 35 keyframes in one HN Replay',
  known_limits: Object.freeze([
    'Only observed keyframe 0x0276 snapshots are emitted; no damage event, source, target, mitigation, or intervening value is inferred.',
    'The three field interpretations and hero participant mapping remain candidates from one HN Replay, not exact-runtime field semantics.',
    'Raw f32 values and their derived floors are both retained; none is a stored integer damage total.',
    'The last keyframe precedes game end, so Replay-tail gaps are retained without interpolation.',
  ]),
});

const HERO_DAMAGE_TAKEN_FROM_CHAMPIONS_SNAPSHOT_CANDIDATE_PROFILE = Object.freeze({
  id: 'rofl-16.19.820.7193-hn-hero-damage-taken-from-champions-keyframe-candidate-v1',
  replay_version: REPLAY_VERSION,
  ...
  payload_length: PAYLOAD_LENGTH,
  decoded_blob_length: BLOB_LENGTH,
  damage_taken_from_champions_f32le_offset_candidate: DAMAGE_TAKEN_FROM_CHAMPIONS_OFFSET,
  evidence_runtime_image_sha256: RUNTIME_IMAGE_SHA256,
  lookup_table_sha256: LOOKUP_TABLE_SHA256,
  evidence_scope: 'exact HN HeroStats route and transform; decoded f32 offset 0x200 correlates after flooring with TOTAL_DAMAGE_TAKEN_FROM_CHAMPIONS Replay tails across 620 participant snapshots from two HN Replays (35 and 27 keyframes)',
  known_limits: Object.freeze([
    'Only observed keyframe 0x0276 snapshots are emitted; no individual damage event, source, target, mitigation, or intervening value is inferred.',
...
const HERO_DAMAGE_SELF_MITIGATED_SNAPSHOT_CANDIDATE_PROFILE = Object.freeze({
  id: 'rofl-16.19.820.7193-hn-hero-damage-self-mitigated-keyframe-candidate-v1',
  replay_version: REPLAY_VERSION,
  ...
  payload_length: PAYLOAD_LENGTH,
  decoded_blob_length: BLOB_LENGTH,
  damage_self_mitigated_f32le_offset_candidate: DAMAGE_SELF_MITIGATED_OFFSET,
  evidence_runtime_image_sha256: RUNTIME_IMAGE_SHA256,
  lookup_table_sha256: LOOKUP_TABLE_SHA256,
  evidence_scope: 'exact HN HeroStats route and transform; decoded f32 offset 0x208 correlates after flooring with TOTAL_DAMAGE_SELF_MITIGATED Replay tails across 620 participant snapshots from two HN Replays (35 and 27 keyframes)',
  known_limits: Object.freeze([
    'Only observed keyframe 0x0276 snapshots are emitted; no individual damage event, source, target, mitigation mechanism, or intervening value is inferred.',
    'The offset 0x208 interpretation and hero participant mapping remain candidates from two HN Replays, not exact-runtime field semantics or proof of actual mitigation.',
    'Raw f32 and its derived floor are retained separately; the floor is not a stored integer damage total.',
    'The last keyframe precedes game end, so Replay-tail gaps are retained without interpolation.',
  ]),
});

const HERO_LONGEST_LIVING_TIME_SNAPSHOT_CANDIDATE_PROFILE = Object.freeze({
  id: 'rofl-16.19.820.7193-hn-hero-longest-living-time-keyframe-candidate-v1',
  replay_version: REPLAY_VERSION,
  ...
  payload_length: PAYLOAD_LENGTH,
  decoded_blob_length: BLOB_LENGTH,
  longest_living_time_f32le_offset_candidate: LONGEST_LIVING_TIME_OFFSET,
  evidence_runtime_image_sha256: RUNTIME_IMAGE_SHA256,
  lookup_table_sha256: LOOKUP_TABLE_SHA256,
  evidence_scope: 'exact HN HeroStats route and transform; decoded f32 offset 0x244 floors correlate with LONGEST_TIME_SPENT_LIVING Replay tails across 910 participant snapshots in three HN Replays (35, 27 and 29 keyframes); final floor matches 10/10, 10/10 and 9/10',
  known_limits: Object.freeze([
    'Only observed keyframe 0x0276 snapshots are emitted; no individual life span, death duration, or intervening value is inferred.',
    'The offset 0x244 interpretation and hero participant mapping remain candidates, not exact-runtime field semantics.',
    'Raw f32 and its derived floor are retained separately; the floor is not a stored integer time.',
    'In the third Replay participant 5 has last snapshot zero versus tail 1706; a candidate death occurs after the last keyframe, but this does not prove the field update rule.',
    'The last keyframe precedes game end, so Replay-tail gaps are retained without interpolation.',
  ]),
});
...
const HERO_TOTAL_HEAL_SNAPSHOT_CANDIDATE_PROFILE = Object.freeze({
  ...
  payload_length: PAYLOAD_LENGTH,
  decoded_blob_length: BLOB_LENGTH,
  total_heal_u32le_offset_candidate: TOTAL_HEAL_OFFSET,
  evidence_runtime_image_sha256: RUNTIME_IMAGE_SHA256,
  lookup_table_sha256: LOOKUP_TABLE_SHA256,
  evidence_scope: 'exact HN HeroStats route and transform; u32 offset 0x234 correlates with participant-aligned TOTAL_HEAL Replay tails across 350 snapshots from 35 keyframes in one HN Replay',
  known_limits: Object.freeze([
    'Only observed keyframe 0x0276 snapshots are emitted; no individual heal event, recipient, HP change, or intervening value is inferred.',
    'The offset 0x234 interpretation and hero participant mapping remain candidates from one HN Replay, not exact-runtime field semantics.',
    'A Replay-tail TOTAL_HEAL correlation does not establish effective healing, post-clamp amount, or overheal.',
    'The last keyframe precedes game end, so Replay-tail gaps are retained without interpolation.',
  ]),
});
...
function decodeHeroKillStatsPayload(payload) {
  const decoded = decodeHeroStatsBlob(payload);
  if (decoded.status !== 'PASS') return decoded;
  return { status: 'PASS', ...Object.fromEntries(KILL_STATS_FIELDS.map(({ candidateKey, offset }) =>
...
function decodeHeroDamageTotalsPayload(payload) {
  const decoded = decodeHeroStatsBlob(payload);
  if (decoded.status !== 'PASS') return decoded;
  const values = Object.fromEntries(DAMAGE_TOTAL_FIELDS.map(({ candidateKey, offset }) =>
    [candidateKey, decoded.blob.readFloatLE(offset)]));
  if (Object.values(values).some((value) => !Number.isFinite(value) || value < 0
    || !Number.isSafeInteger(Math.floor(value)))) {
    return { status: 'DECODE_FAILED',
      error: 'HeroStats damage offsets 0x1d0/0x1e0/0x1f0 must be finite nonnegative safe-range f32 values' };
  }
  return { status: 'PASS', ...values,
    ...Object.fromEntries(DAMAGE_TOTAL_FIELDS.map(({ candidateKey, floorKey }) =>
      [floorKey, Math.floor(values[candidateKey])])) };
}

function decodeHeroDamageTakenFromChampionsPayload(payload) {
  return decodeHeroFloatTailPayload(payload, DAMAGE_TAKEN_FROM_CHAMPIONS_OFFSET,
    'damage_taken_from_champions_raw_f32_candidate',
    'damage_taken_from_champions_floor_candidate', 'damage taken from champions');
}

function decodeHeroDamageSelfMitigatedPayload(payload) {
  return decodeHeroFloatTailPayload(payload, DAMAGE_SELF_MITIGATED_OFFSET,
    'damage_self_mitigated_raw_f32_candidate',
    'damage_self_mitigated_floor_candidate', 'damage self mitigated');
}

function decodeHeroLongestLivingTimePayload(payload) {
  return decodeHeroFloatTailPayload(payload, LONGEST_LIVING_TIME_OFFSET,
...
function assessHeroDamageTotalsSnapshotTail(replay) {
  const assessments = DAMAGE_TOTAL_FIELDS.map(({ tailField }) => assessHeroStatsTail(replay, tailField));
  const failure = assessments.find((row) => row.status !== 'PASS');
  if (failure) return { ...failure, required_fields: assessments };
  return { ...assessments[0], required_fields: assessments,
    valuesByField: Object.fromEntries(assessments.map(({ field, values }) => [field, values])) };
...
function assessHeroDamageTakenFromChampionsSnapshotTail(replay) {
  return assessHeroStatsTail(replay, 'TOTAL_DAMAGE_TAKEN_FROM_CHAMPIONS');
}

function assessHeroDamageSelfMitigatedSnapshotTail(replay) {
  return assessHeroStatsTail(replay, 'TOTAL_DAMAGE_SELF_MITIGATED');
}
...
function decodeHeroKillStatsFromScan(replay, scan) {
  const profile = HERO_KILL_STATS_SNAPSHOT_CANDIDATE_PROFILE;
  const collected = collectHeroStatsSnapshotCandidates(replay, {
    profile,
    assessTail: assessHeroKillStatsSnapshotTail,
    decodePayload: decodeHeroKillStatsPayload,
    valueKey: KILL_STATS_FIELDS[0].candidateKey,
    tailProjection: (value) => value,
  }, scan);
  if (collected.status !== 'CANDIDATE') return collected;
  const { observations, observedDeclines, previous, lastTimes, tailValues, gameLengthMs, ...base } = collected;
  const tail = assessHeroKillStatsSnapshotTail(replay);
  const previousValues = Object.fromEntries(KILL_STATS_FIELDS.map(({ candidateKey }) =>
  ...
  for (const observation of observations) {
    ...
    const values = {};
    const confidence = {};
    for (const { tailField, candidateKey } of KILL_STATS_FIELDS) {
    ...
    events.push(snapshotEvent(replay, profile, observation,
      'HERO_KILL_STATS_SNAPSHOT_CANDIDATE',
      'CANDIDATE_EXACT_ROUTE_ONE_REPLAY_SIX_FIELD_TAIL_BOUND', values, confidence));
  }
  const tailGaps = Array.from({ length: 10 }, (_, index) => ({
    participant_id_candidate: index + 1,
    last_snapshot_replay_time_ms: lastTimes[index],
    field_gaps: Object.fromEntries(KILL_STATS_FIELDS.map(({ tailField, candidateKey }) => [
      tailField,
  ...
  return {
    ...base,
    evidence_status: 'CANDIDATE_EXACT_ROUTE_ONE_REPLAY_SIX_FIELD_TAIL_BOUND',
    evidence_runtime_image_sha256: RUNTIME_IMAGE_SHA256,
    final_tail_values: tail.valuesByField,
    observed_last_values: previousValues,
    tail_gaps: tailGaps,
    tail_gap_totals: Object.fromEntries(KILL_STATS_FIELDS.map(({ tailField }) => [
      tailField,
...
function decodeHeroDamageTotalsFromScan(replay, scan) {
  const profile = HERO_DAMAGE_TOTALS_SNAPSHOT_CANDIDATE_PROFILE;
  const collected = collectHeroStatsSnapshotCandidates(replay, {
    profile,
    assessTail: assessHeroDamageTotalsSnapshotTail,
    decodePayload: decodeHeroDamageTotalsPayload,
    valueKey: DAMAGE_TOTAL_FIELDS[0].candidateKey,
    tailProjection: Math.floor,
  }, scan);
  if (collected.status !== 'CANDIDATE') return collected;
  const { observations, observedDeclines, previous, lastTimes, tailValues, gameLengthMs, ...base } = collected;
  const tail = assessHeroDamageTotalsSnapshotTail(replay);
  const previousValues = Object.fromEntries(DAMAGE_TOTAL_FIELDS.map(({ candidateKey }) =>
    [candidateKey, Array(10).fill(null)]));
  const events = [];
  const fail = (error) => ({ profile_id: profile.id, input_packet_id: PACKET_ID,
    status: 'DECODE_FAILED', event_count: null, input_count: base.input_count,
  ...
  for (const observation of observations) {
    ...
    const values = {};
    const confidence = {};
    for (const { tailField, candidateKey, floorKey } of DAMAGE_TOTAL_FIELDS) {
      const value = observation.decoded[candidateKey];
      const floor = observation.decoded[floorKey];
      if (previousValues[candidateKey][index] !== null
        && value < previousValues[candidateKey][index]) {
        return fail(`HN participant ${observation.participantId} has a decreasing observed ${tailField} snapshot`);
      }
      if (floor > tail.valuesByField[tailField][index]) {
        return fail(`HN participant ${observation.participantId} exceeds Replay tail ${tailField}`);
      }
      previousValues[candidateKey][index] = value;
      values[candidateKey] = value;
      values[floorKey] = floor;
      confidence[candidateKey] = 'CANDIDATE_ONE_REPLAY_TAIL_CORRELATION';
      confidence[floorKey] = 'DERIVED_FROM_CANDIDATE';
    }
    events.push(snapshotEvent(replay, profile, observation,
      'HERO_DAMAGE_TOTALS_SNAPSHOT_CANDIDATE',
  ...
  const tailGaps = Array.from({ length: 10 }, (_, index) => ({
    participant_id_candidate: index + 1,
    last_snapshot_replay_time_ms: lastTimes[index],
    field_gaps: Object.fromEntries(DAMAGE_TOTAL_FIELDS.map(({ tailField, candidateKey }) => [
      tailField,
      { last_snapshot_raw_f32_candidate: previousValues[candidateKey][index],
        last_snapshot_floor_candidate: Math.floor(previousValues[candidateKey][index]),
        final_tail: tail.valuesByField[tailField][index],
        unobserved_tail_floor_gap: tail.valuesByField[tailField][index]
          - Math.floor(previousValues[candidateKey][index]) },
    ])),
    unobserved_tail_time_ms: gameLengthMs === null ? null : gameLengthMs - lastTimes[index],
  }));
  return {
    ...base,
    evidence_status: 'CANDIDATE_EXACT_ROUTE_ONE_REPLAY_THREE_DAMAGE_TAIL_CORRELATIONS',
    evidence_runtime_image_sha256: RUNTIME_IMAGE_SHA256,
    final_tail_values: tail.valuesByField,
    observed_last_raw_f32_values: previousValues,
    tail_gaps: tailGaps,
    events,
  };
}
...
function decodeHeroTotalHealFromScan(replay, scan) {
  ...
  const collected = collectHeroStatsSnapshotCandidates(replay, {
    profile,
    assessTail: assessHeroTotalHealSnapshotTail,
    decodePayload: decodeHeroTotalHealPayload,
    valueKey: 'total_heal_candidate',
    tailProjection: (value) => value,
  }, scan);
  if (collected.status !== 'CANDIDATE') return collected;
  const { observations, observedDeclines, previous, lastTimes, tailValues, gameLengthMs, ...base } = collected;
  const tailGaps = tailValues.map((finalValue, index) => ({
    participant_id_candidate: index + 1,
    last_snapshot_replay_time_ms: lastTimes[index],
    last_snapshot_total_heal_candidate: previous[index],
    final_total_heal_tail: finalValue,
    unobserved_tail_gap: finalValue - previous[index],
    unobserved_tail_time_ms: gameLengthMs === null ? null : gameLengthMs - lastTimes[index],
  }));
  return {
    ...base,
    evidence_status: 'CANDIDATE_EXACT_ROUTE_ONE_REPLAY_TOTAL_HEAL_TAIL_CORRELATION',
    evidence_runtime_image_sha256: RUNTIME_IMAGE_SHA256,
    final_total_heal_tail: tailValues,
    observed_last_total_heal_candidate: previous,
    tail_gaps: tailGaps,
    tail_gap_total: tailGaps.reduce((sum, gap) => sum + gap.unobserved_tail_gap, 0),
    events: observations.map((observation) => snapshotEvent(replay, profile, observation,
      'HERO_TOTAL_HEAL_SNAPSHOT_CANDIDATE',
      'CANDIDATE_EXACT_ROUTE_ONE_REPLAY_TOTAL_HEAL_TAIL_CORRELATION',
      { total_heal_candidate: observation.value },
      { total_heal_candidate: 'CANDIDATE_ONE_REPLAY_TAIL_CORRELATION' })),
  };
}
...
function decodeHeroFloatTailFromScan(replay, scan, spec) {
  ...
  return {
    ...
    events: observations.map((observation) => snapshotEvent(replay, profile, observation,
      ...
      { [rawKey]: rawFieldConfidence,
        [floorKey]: 'DERIVED_FROM_CANDIDATE' })),
  };
}

function decodeHeroDamageTakenFromChampionsFromScan(replay, scan) {
  return decodeHeroFloatTailFromScan(replay, scan, {
    profile: HERO_DAMAGE_TAKEN_FROM_CHAMPIONS_SNAPSHOT_CANDIDATE_PROFILE,
    assessTail: assessHeroDamageTakenFromChampionsSnapshotTail,
    decodePayload: decodeHeroDamageTakenFromChampionsPayload,
    rawKey: 'damage_taken_from_champions_raw_f32_candidate',
    floorKey: 'damage_taken_from_champions_floor_candidate',
    eventType: 'HERO_DAMAGE_TAKEN_FROM_CHAMPIONS_SNAPSHOT_CANDIDATE',
    evidenceStatus: 'CANDIDATE_EXACT_ROUTE_TWO_REPLAY_DAMAGE_TAKEN_FROM_CHAMPIONS_TAIL_CORRELATION',
    rawFieldConfidence: 'CANDIDATE_TWO_REPLAY_TAIL_CORRELATION',
  });
}

function decodeHeroDamageSelfMitigatedFromScan(replay, scan) {
  return decodeHeroFloatTailFromScan(replay, scan, {
    profile: HERO_DAMAGE_SELF_MITIGATED_SNAPSHOT_CANDIDATE_PROFILE,
    assessTail: assessHeroDamageSelfMitigatedSnapshotTail,
    decodePayload: decodeHeroDamageSelfMitigatedPayload,
    rawKey: 'damage_self_mitigated_raw_f32_candidate',
    floorKey: 'damage_self_mitigated_floor_candidate',
    eventType: 'HERO_DAMAGE_SELF_MITIGATED_SNAPSHOT_CANDIDATE',
    evidenceStatus: 'CANDIDATE_EXACT_ROUTE_TWO_REPLAY_DAMAGE_SELF_MITIGATED_TAIL_CORRELATION',
    rawFieldConfidence: 'CANDIDATE_TWO_REPLAY_TAIL_CORRELATION',
  });
}

function decodeHeroLongestLivingTimeFromScan(replay, scan) {
  return decodeHeroFloatTailFromScan(replay, scan, {
    profile: HERO_LONGEST_LIVING_TIME_SNAPSHOT_CANDIDATE_PROFILE,
    assessTail: assessHeroLongestLivingTimeSnapshotTail,
    decodePayload: decodeHeroLongestLivingTimePayload,
    rawKey: 'longest_living_time_raw_f32_candidate',
    floorKey: 'longest_living_time_floor_candidate',
    eventType: 'HERO_LONGEST_LIVING_TIME_SNAPSHOT_CANDIDATE',
    evidenceStatus: 'CANDIDATE_EXACT_ROUTE_THREE_REPLAY_LONGEST_LIVING_TIME_TAIL_CORRELATION',
...
function decodeHeroStatsSnapshotCandidateSet(replay, capabilities, precollectedScan) {
  ...
  if (selected.has('hero_damage_totals_snapshot')) {
    outcomes.hero_damage_totals_snapshot = decodeHeroDamageTotalsFromScan(replay, scan);
  }
  if (selected.has('hero_damage_taken_from_champions_snapshot')) {
    outcomes.hero_damage_taken_from_champions_snapshot =
      decodeHeroDamageTakenFromChampionsFromScan(replay, scan);
  }
  if (selected.has('hero_damage_self_mitigated_snapshot')) {
    outcomes.hero_damage_self_mitigated_snapshot =
      decodeHeroDamageSelfMitigatedFromScan(replay, scan);
  }
  if (selected.has('hero_longest_living_time_snapshot')) {
...
function decodeHeroKillStatsSnapshotCandidates(replay) {
  return decodeHeroStatsSnapshotCandidateSet(replay,
...
function decodeHeroDamageTotalsSnapshotCandidates(replay) {
  return decodeHeroStatsSnapshotCandidateSet(replay,
    ['hero_damage_totals_snapshot']).hero_damage_totals_snapshot;
}

function decodeHeroDamageTakenFromChampionsSnapshotCandidates(replay) {
  return decodeHeroStatsSnapshotCandidateSet(replay,
    ['hero_damage_taken_from_champions_snapshot']).hero_damage_taken_from_champions_snapshot;
}

function decodeHeroDamageSelfMitigatedSnapshotCandidates(replay) {
  return decodeHeroStatsSnapshotCandidateSet(replay,
    ['hero_damage_self_mitigated_snapshot']).hero_damage_self_mitigated_snapshot;
}

function decodeHeroLongestLivingTimeSnapshotCandidates(replay) {
  return decodeHeroStatsSnapshotCandidateSet(replay,
...
module.exports = {
  ...
  decodeHeroDamageTakenFromChampionsPayload,
  decodeHeroDamageTakenFromChampionsSnapshotCandidates,
  decodeHeroDamageSelfMitigatedPayload,
