...
const CHAMPION_DOUBLE_KILL_EVENT_PACKET_821_PROFILE = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-on-champion-double-kill-event-packet-runtime-candidate-v1',
  ...
  child_event_name: 'OnChampionDoubleKill',
  evidence_runtime_image_sha256: IMAGE_SHA256,
  evidence_packet_callback_rva: '0x4ce3a0',
  evidence_child_callback_registration_lea_rva: '0x271350',
  evidence_child_registration_id_write_rva: '0x27135b',
  evidence_child_callback_rva: '0x2b9de0',
  evidence_event_name_table_rva: '0x1ef7330',
  runtime_image_required: true,
  evidence_scope: 'exact 821 image names child 0x000b OnChampionDoubleKill; 654/654 observed 104-byte OnEvent packets natively consume across 11 KR Replays, with 63 targets and 591 same-length foreign-child controls',
  known_limits: Object.freeze([
    'The event name is an exact-image label; this packet marker does not prove an effective double kill or any state transition.',
    'The registered callback obtains a value through a context-object virtual accessor; no direct child-blob field mapping or gameplay role is established.',
    'Native child-blob bytes are retained only by digest; no child u32 is given a gameplay role or emitted as a semantic field.',
    'The 104-byte parent shape also carries three foreign child IDs, which must be excluded by native child identity.',
...
function decodeChampionDoubleKillEventPacketCandidates821(replay, {
  runtimeImagePath, pythonExecutable, precollected,
} = {}) {
  const profile = CHAMPION_DOUBLE_KILL_EVENT_PACKET_821_PROFILE;
  const base = {
    profile_id: profile.id,
    input_packet_id: PACKET_ID,
    input_packet_scope: 'child_000b_length_104',
    child_event_id: CHILD_EVENT_ID,
    evidence_runtime_image_sha256: IMAGE_SHA256,
  };
  ...
  if (replay?.header?.version !== REPLAY_VERSION) {
    return fail('UNSUPPORTED', `OnChampionDoubleKill packet candidate supports only ${REPLAY_VERSION}`);
  }
  ...
  if (observedPacketCount > MAX_TOTAL_PACKETS) {
    return fail('UNSUPPORTED', `OnChampionDoubleKill runtime input exceeds ${MAX_TOTAL_PACKETS} packets`, {
      observed_packet_count_minimum: observedPacketCount,
  ...
  if (!Array.isArray(rows)) {
    return fail('DECODE_FAILED', '821 OnChampionDoubleKill route scan returned no packet rows', {
      scanned_block_count: scannedBlockCount,
    });
  }
  if (rows.length === 0) {
  ...
  for (const { block, chunk } of rows) {
    if (chunk.stream_tag !== 1 || chunk.stream !== 'game_chunk'
      ...
      return failed('DECODE_FAILED', '0x040a OnChampionDoubleKill packet framing differs from observed KR scope', {
  ...
  const script = path.resolve(__dirname, '..', '..', 'scripts',
    'decode_champion_double_kill_event_packet_16_19_821.py');
  const events = [];
  ...
  for (let start = 0; start < rows.length; start += MAX_BATCH_PACKETS) {
    ...
    if (Buffer.byteLength(request) > MAX_REQUEST_BYTES) {
      return failed('UNSUPPORTED', 'OnChampionDoubleKill runtime request exceeds bounded input size', {
    ...
    if (run.error || run.status !== 0) {
      ...
      return failed(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `exact runtime OnChampionDoubleKill packet decoder failed: ${detail}`, {
    ...
    try {
      ...
      return failed('DECODE_FAILED', `runtime OnChampionDoubleKill output is not JSON: ${error.message}`, {
    ...
    if (decoded?.status !== 'PASS' || decoded.runtime_image_sha256 !== IMAGE_SHA256
      ...
      return failed('DECODE_FAILED', 'runtime OnChampionDoubleKill output identity or packet count differs', {
        runtime_image_used: start > 0, runtime_image_status: 'EXECUTION_FAILED',
      });
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      if (!valid) {
        return failed('DECODE_FAILED', `runtime OnChampionDoubleKill packet ${start + index} did not fully decode`, {
          runtime_image_status: 'MATCHED_USED', runtime_image_used: true,
          runtime_image_sha256: IMAGE_SHA256, first_failed_packet_ref: ref,
          runtime_packet_result: row ?? null,
        });
      }
      if (row.event_id !== CHILD_EVENT_ID) {
...
module.exports = {
  ...
  decodeChampionDoubleKillEventPacketCandidates821,
};
