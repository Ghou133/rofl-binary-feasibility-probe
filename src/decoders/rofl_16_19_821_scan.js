...
function create821ScanCollector(replay, selectedCapabilities) {
  ...
  return Object.freeze({
    observe(block, chunk) {
      ...
      if (selectsDampenerDieEvent
          && block.packet_id === 0x040a && block.payload_length === 116) {
        dampenerDieEventPacketCount += 1;
        if (rows.dampener_die_event_packet.length < MAX_DAMPENER_DIE_EVENT_PACKET_ROWS) {
          rows.dampener_die_event_packet.push(copyRow(block, chunk));
        }
      }
      ...
      if (selectsHqKillEvent
          && block.packet_id === 0x040a && block.payload_length === 116) {
        hqKillEventPacketCount += 1;
        if (rows.hq_kill_event_packet.length < MAX_HQ_KILL_EVENT_PACKET_ROWS) {
          rows.hq_kill_event_packet.push(copyRow(block, chunk));
        }
      }
      ...
      if (selectsBuffUpdateNumCounter && block.packet_id === 0x0194) {
        buffUpdateNumCounterPacketCount += 1;
        if (rows.npc_buff_update_num_counter_packet.length
      ...
      if (selectsBuffReplace && block.packet_id === 0x01ad) {
        buffReplacePacketCount += 1;
        if (rows.npc_buff_replace_packet.length < MAX_BUFF_REPLACE_PACKET_ROWS) {
          rows.npc_buff_replace_packet.push(copyRow(block, chunk));
        }
      }
      if (selectsSetSpellTimerFromBuff && block.packet_id === 0x00fd) {
        setSpellTimerFromBuffPacketCount += 1;
        if (rows.set_spell_timer_from_buff_packet.length
      ...
      if (selectsIncrementMinionKills && block.packet_id === 0x03a7) {
        incrementMinionKillsPacketCount += 1;
        if (rows.increment_minion_kills_packet.length
            < MAX_INCREMENT_MINION_KILLS_PACKET_ROWS) {
...
function rowsFor821Capability(replay, token, capability) {
  ...
  if (capability === 'dampener_die_event_packet'
      && bound.dampenerDieEventPacketCount > MAX_DAMPENER_DIE_EVENT_PACKET_ROWS) {
    return {
      observed_packet_count_minimum: bound.dampenerDieEventPacketCount,
      scanned_block_count: bound.blockCount,
    };
  }
  ...
  if (capability === 'hq_kill_event_packet'
      && bound.hqKillEventPacketCount > MAX_HQ_KILL_EVENT_PACKET_ROWS) {
    return {
      observed_packet_count_minimum: bound.hqKillEventPacketCount,
      scanned_block_count: bound.blockCount,
    };
  }
  ...
  if (capability === 'npc_buff_update_num_counter_packet'
      && bound.buffUpdateNumCounterPacketCount
        > MAX_BUFF_UPDATE_NUM_COUNTER_PACKET_ROWS) {
    return {
  ...
  if (capability === 'npc_buff_replace_packet'
      && bound.buffReplacePacketCount > MAX_BUFF_REPLACE_PACKET_ROWS) {
    return {
      observed_packet_count_minimum: bound.buffReplacePacketCount,
      scanned_block_count: bound.blockCount,
    };
  }
  if (capability === 'set_spell_timer_from_buff_packet'
      && bound.setSpellTimerFromBuffPacketCount
        > MAX_SET_SPELL_TIMER_FROM_BUFF_PACKET_ROWS) {
    return {
  ...
  if (capability === 'increment_minion_kills_packet'
