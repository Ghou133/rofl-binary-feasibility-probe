...
const CAPABILITY = 'revive_ally_event_packet';
const PACKET_ID = 0x040a;
const CHILD_EVENT_ID = 0x002c;
const RAW_EVENT_ID_HEX = '0x49ca';
...
const REVIVE_ALLY_EVENT_PACKET_821_PROFILE = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-on-revive-ally-event-packet-runtime-candidate-v1',
  ...
  child_event_name: 'OnReviveAlly',
  evidence_runtime_image_sha256: IMAGE_SHA256,
  evidence_event_name_table_rva: '0x1ef7a10',
  evidence_registration_id_write_rva: '0x27253b',
  evidence_callback_rva: '0x2d1590',
  runtime_image_required: true,
  evidence_scope: '11 exact-build KR Replays: three target-shape 0x040a/16 packets in one Replay and nine observed same-length foreign-child controls',
  known_limits: Object.freeze([
    'The event name is an exact-image label. A packet does not prove a revive, actor role, or game-state transition.',
    'Native child +0x04 is an anonymous u32; its role and any effect are UNKNOWN.',
    'The registered callback reads parameter-object fields; their game roles are not established.',
    'The 16-byte parent route has same-length foreign children. The observed raw fingerprint only selects native checks; the native child ID is authoritative.',
    'Three target-shape packets occur in one of 11 KR Replays. Absence in another Replay is PROFILE_UNAVAILABLE, not an observed zero count.',
...
function rawShape(payload) {
  if (payload.subarray(1, 5).equals(Buffer.from('bdb94b3d', 'hex'))
      && payload[PAYLOAD_LENGTH - 1] === 0xca) return 'target';
  if (payload.subarray(1, 5).equals(Buffer.from('bdb9b33d', 'hex'))
      && payload[PAYLOAD_LENGTH - 1] === 0xe4) return 'observed_foreign_0017';
  return null;
}

function decodeReviveAllyEventPacketCandidates821(replay, {
  ...
  const base = {
    ...
    input_packet_scope: 'child_002c_length_16',
    child_event_id: CHILD_EVENT_ID,
    evidence_runtime_image_sha256: IMAGE_SHA256,
  };
  ...
  if (replay?.header?.version !== REPLAY_VERSION) {
    return fail('UNSUPPORTED', `OnReviveAlly packet candidate supports only ${REPLAY_VERSION}`);
  }
  ...
  if (observedPacketCount > MAX_TOTAL_PACKETS) {
    return fail('UNSUPPORTED', `OnReviveAlly runtime input exceeds ${MAX_TOTAL_PACKETS} packets`, {
      observed_packet_count_minimum: observedPacketCount,
  ...
  if (!Array.isArray(rows)) {
    return fail('DECODE_FAILED', '821 OnReviveAlly route scan returned no packet rows', {
      scanned_block_count: scannedBlockCount,
    });
  }
  const target = [];
  ...
  for (const { block, chunk } of rows) {
    ...
    else if (shape === 'observed_foreign_0017') {
      excludedForeign.push(packetRef(replay, block, chunk));
    } else {
      return fail('DECODE_FAILED', 'unrecognized 0x040a/16 OnEvent raw child fingerprint', {
        scanned_block_count: scannedBlockCount,
  ...
  if (target.length === 0) {
    return fail('PROFILE_UNAVAILABLE', 'KR 0x040a/16 OnReviveAlly raw shape is absent', {
  ...
  for (let start = 0; start < target.length; start += MAX_BATCH_PACKETS) {
    ...
    if (Buffer.byteLength(request) > MAX_REQUEST_BYTES) {
      return failed('UNSUPPORTED', 'OnReviveAlly runtime request exceeds bounded input size', {
    ...
    if (run.error || run.status !== 0) {
      ...
      return failed(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `exact runtime OnReviveAlly packet decoder failed: ${detail}`, {
    ...
    try {
      ...
      return failed('DECODE_FAILED', `runtime OnReviveAlly packet output is not JSON: ${error.message}`, {
    ...
    if (decoded?.status !== 'PASS' || decoded.runtime_image_sha256 !== IMAGE_SHA256
      ...
      return failed('DECODE_FAILED', 'runtime OnReviveAlly output identity or packet count differs', {
        runtime_image_used: start > 0, runtime_image_status: 'EXECUTION_FAILED',
      });
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      if (!valid || sha256(blob) !== row.event_blob_sha256
        ...
        return failed('DECODE_FAILED', `runtime OnReviveAlly packet ${start + index} did not match exact child scope`, {
...
module.exports = {
  REVIVE_ALLY_EVENT_PACKET_821_PROFILE,
  decodeReviveAllyEventPacketCandidates821,
};
