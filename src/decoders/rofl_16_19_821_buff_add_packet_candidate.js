...
const CALLBACK_TABLE_SHA256 = Object.freeze({
  opaque_u32_0x10: 'a90f34d17c8715929574f708ea279badb13f527d6d025a6138fb179b7329c84b',
...
const NPC_BUFF_ADD_PACKET_CANDIDATE_PROFILE_821 = Object.freeze({
  id: 'rofl-16.19.821.7343-kr-buff-add2-packet-runtime-candidate-v1',
  ...
  evidence_scope: 'exact 821 image registration/callback and 34527/34527 fully consumed route packets in one KR Replay; 584/584 route-shape samples from 11 KR Replays',
  known_limits: Object.freeze([
    'The packet class name and decoded fields do not prove a successful buff application.',
    'Object offsets 0x10 and 0x14 remain anonymous; no owner, buff identity, target, application, duration or lifecycle is inferred.',
    'Only the observed game/keyframe 821 stream and packet-length combinations are accepted.',
...
function decodeNpcBuffAddPacketCandidates821(replay, {
  runtimeImagePath, pythonExecutable, precollected,
} = {}) {
  const profile = NPC_BUFF_ADD_PACKET_CANDIDATE_PROFILE_821;
  const base = {
  ...
  if (replay?.header?.version !== REPLAY_VERSION) {
    return fail('UNSUPPORTED', `BuffAdd2 packet candidate supports only ${REPLAY_VERSION}`);
  }
  ...
  if (selected.observed_packet_count_minimum > MAX_TOTAL_PACKETS) {
    return fail('UNSUPPORTED', `BuffAdd2 runtime input exceeds ${MAX_TOTAL_PACKETS} packets`, {
      observed_packet_count_minimum: selected.observed_packet_count_minimum,
      scanned_block_count: scannedBlockCount,
    });
  }
  if (rows.length === 0) {
    return fail('PROFILE_UNAVAILABLE', 'KR 0x00ae BuffAdd2 route is absent', {
  ...
  if (inputCount > MAX_TOTAL_PACKETS) {
    return failed('UNSUPPORTED', `BuffAdd2 runtime input exceeds ${MAX_TOTAL_PACKETS} packets`);
  }
  for (const { block, chunk } of rows) {
    const param = block.param >>> 0;
    if (!OBSERVED_PAYLOAD_LENGTHS_BY_STREAM[chunk.stream_tag]
      ...
      return failed('DECODE_FAILED', '0x00ae packet framing differs from observed KR scope', {
        first_failed_packet_ref: packetRef(replay, block, chunk),
      });
    }
    if (!OBSERVED_PAYLOAD_LENGTHS_BY_STREAM[chunk.stream_tag].has(block.payload_length)) {
      return failed('DECODE_FAILED', '0x00ae stream/payload length differs from observed KR scope', {
        first_failed_packet_ref: packetRef(replay, block, chunk),
      });
    }
  }
  ...
  const script = path.resolve(__dirname, '..', '..', 'scripts',
    'decode_buff_add_packet_16_19_821.py');
  const events = [];
  for (let start = 0; start < rows.length; start += MAX_BATCH_PACKETS) {
    ...
    if (Buffer.byteLength(request) > MAX_REQUEST_BYTES) {
      return failed('UNSUPPORTED', 'BuffAdd2 runtime request exceeds bounded input size', {
    ...
    if (run.error || run.status !== 0) {
      ...
      return failed(missingPython ? 'MISSING_INPUT' : 'DECODE_FAILED',
        `exact runtime BuffAdd2 packet decoder failed: ${detail}`, {
    ...
    try {
      ...
      return failed('DECODE_FAILED', `runtime BuffAdd2 packet output is not JSON: ${error.message}`, {
    ...
    if (decoded?.status !== 'PASS' || decoded.runtime_image_sha256 !== IMAGE_SHA256
      ...
      return failed('DECODE_FAILED', 'runtime BuffAdd2 output identity or packet count differs', {
        runtime_image_used: start > 0, runtime_image_status: 'EXECUTION_FAILED',
      });
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      if (row?.status !== 'DECODED' || row.input_index !== index
          ...
          || row.opaque_u8_0x14 > 0xff
          || !/^[0-9a-f]{8}$/.test(row.raw_u32_bytes_hex)
          || !/^[0-9a-f]{2}$/.test(row.raw_u8_byte_hex)) {
        return failed('DECODE_FAILED', `runtime BuffAdd2 packet ${start + index} did not fully decode`, {
...
module.exports = {
  NPC_BUFF_ADD_PACKET_CANDIDATE_PROFILE_821,
  decodeNpcBuffAddPacketCandidates821,
};
