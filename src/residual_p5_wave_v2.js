...
const ROUTE_IDS = Object.freeze([
  0x0288, 0x0093, 0x00bf, 0x04b2,
  0x017d, 0x02e5, 0x01b2, 0x0041,
]);
...
const FAMILY_BY_ROUTE = Object.freeze({
  '0x0288': 'PROTECTED_28_BYTE_VARIABLE_BRANCH_CARRIER',
  '0x0093': 'PROTECTED_24_BYTE_COMPACT_BRANCH_CARRIER_A',
  '0x00bf': 'PROTECTED_24_BYTE_COMPACT_BRANCH_CARRIER_B',
  '0x04b2': 'PROTECTED_24_BYTE_COMPACT_BRANCH_CARRIER_C',
  '0x017d': 'PROTECTED_40_BYTE_VARIABLE_BRANCH_CARRIER',
  '0x02e5': 'PROTECTED_28_BYTE_COMPACT_BRANCH_CARRIER',
  '0x01b2': 'PROTECTED_168_BYTE_VARIABLE_RECORD_CARRIER',
  '0x0041': 'EXTERNAL_STATE_DEPENDENT_112_BYTE_VARIABLE_RECORD_CARRIER',
});
...
function distinctPayloadRow(event, payloadHash, payload) {
  return {
    schema: 'RESIDUAL_P5_DISTINCT_PAYLOAD_V2',
    schema_version: 2,
    exact_build: EXACT_BUILD,
    replay_version: EXACT_BUILD,
    packet_id: event.packet_id,
    packet_type: packetHex(event.packet_id),
...
function collectReplay(replayPath, states, anchors) {
  ...
  invariant(Object.hasOwn(SAFE_REPLAYS, basename), `replay outside explicit allowlist: ${basename}`);
  ...
  const walk = walkBlocks(replay, (block, chunk) => {
    ...
    const families = ANCHOR_BY_ID.get(block.packet_id);
    if (families) {
      for (const family of families) {
        if (!anchors.get(family).has(replay.source_sha256)) anchors.get(family).set(replay.source_sha256, []);
    ...
    if (state) {
      ...
      increment(state.raw_param_classes, paramClass(event.raw_param));
      ...
      if (!state.payloads.has(payloadHash)) {
        state.payloads.set(payloadHash, distinctPayloadRow(event, payloadHash, block.payload));
      } else {
...
function temporalSummary(events) {
  ...
  return {
    ...
    same_timestamp_transition_rate: round(sameTime / Math.max(1, transitionCount)),
  };
}
...
function summarizeRoute(state, anchors) {
  ...
  const anchorRows = {};
  for (const [family, byReplay] of anchors) anchorRows[family] = anchorCorrelation(state.events, byReplay);
  return {
    ...
    anchor_correlations_with_shifted_controls: anchorRows,
...
function buildProfiles(staticRecovery) {
  return ROUTE_IDS.map((packetId) => {
    ...
    const route = staticRecovery.routes[packetType];
    invariant(route?.factory_chain, `static recovery missing ${packetType}`);
    const factory = route.factory_chain;
    return {
      schema: 'RESIDUAL_P5_EXACT_NATIVE_PROFILE_V2',
      schema_version: 2,
      exact_build: EXACT_BUILD,
      exact_runtime_image_sha256: RUNTIME_SHA256,
      static_identity: route,
      profile: {
        id: `residual-p5-${packetType.slice(2)}-exact-runtime-v2`,
...
function classifyNativeFailure(message) {
  ...
  const external = /invalid memory access|unmapped|GS:|TLS|heap/i.test(text);
  return {
    class: external ? 'EXTERNAL_RUNTIME_HEAP_OR_TLS_STATE' : 'UNCLASSIFIED_NATIVE_FAILURE',
    fault_rip: rip,
...
function analyzeNative(nativeRowsByRoute, evidence, staticRecovery) {
  ...
  for (const packetId of ROUTE_IDS) {
    ...
    for (const row of failed) {
      ...
      failures.set(key, (failures.get(key) || 0) + Number(row.occurrence_count || 1));
    }
    const failureClasses = [...failures.entries()].map(([key, occurrenceWeight]) => ({
    ...
    routes[packetType] = {
      ...
      all_failures_external_runtime_state: failed.length === 0
        || failureClasses.every((row) => row.class === 'EXTERNAL_RUNTIME_HEAP_OR_TLS_STATE'),
...
function strongestPair(matrix, sourceRoute) {
  return matrix.filter((row) => row.source_route === sourceRoute)[0] || null;
}
...
function buildDecisions(evidence, staticRecovery, nativeAnalysis) {
  const routeDecisions = ROUTE_IDS.map((packetId) => {
    ...
    const localComplete = runtime.validations.all_factory_checks_pass
      && runtime.validations.deserializer_object_access_scan_executed
      && runtime.validations.direct_xref_surfaces_scanned
      && runtime.validations.full_pdata_immediate_scan_executed
    ...
    const external = native.conserved_failure_occurrence_weight > 0
      ? 'Authorized exact-build live heap/TLS snapshot spanning the recorded fault RIP/address, plus the heap callback-tree node for this packet ID.'
      : 'Controlled exact-build producer/consumer trace with plaintext business-state oracle, plus the heap callback-tree node if a symbolic receive owner is required.';
    return {
      decision_id: `ROUTE_${packetType}_RESIDUAL_P5_WAVE_V2`,
      packet_id: packetId,
      packet_discriminator: packetType,
      decision: 'REPURPOSE',
      domain: 'unmapped_heap_callback_protocol',
      capability: `EXACT_ROUTE_${packetType.slice(2).toUpperCase()}_STRUCTURAL_CARRIER`,
      hypothesis: FAMILY_BY_ROUTE[packetType],
      semantic_claim: null,
      ...
      observed_occurrence_count: raw.count,
      ...
      reverse_engineering_steps: [
        ...
        'CONSTRUCTOR_VTABLE_DESERIALIZER_DIRECT_XREF_AND_CALL_TARGET_SCAN',
        'MAKEFUNCTION_CALLBACK_MAP_AND_FULL_PDATA_IMMEDIATE_REFERENCE_SCAN',
      ...
      actionable_hypotheses: localComplete ? [] : [
        'Resolve the local static/native validation that remains false in this report.',
      ],
      next_required_evidence: localComplete ? [external] : [
      ...
      external_only_gate: {
        ...
        required_evidence: localComplete ? external : null,
      },
    };
  });
  ...
  const domainDecision = {
    decision_id: 'DOMAIN_UNMAPPED_HEAP_CALLBACK_PROTOCOL_RESIDUAL_P5_V2',
    domain: 'unmapped_heap_callback_protocol',
  ...
  const decisionVocabulary = ['PROMOTE', 'REPURPOSE', 'KEEP_CANDIDATE', 'REJECT'];
  const countDecisions = (rows) => Object.fromEntries(decisionVocabulary
  ...
  return {
    ...
    decision_counts: countDecisions(routeDecisions),
    decision_counts_by_level: {
      route: countDecisions(routeDecisions),
      capability: countDecisions(capabilityDecisions),
      domain: countDecisions([domainDecision]),
    },
...
function validateDecisionBundle(bundle) {
  invariant(bundle.schema === 'RESIDUAL_P5_WAVE_MACHINE_DECISIONS_V2', 'decision schema mismatch');
  ...
  for (const group of ['route_decisions', 'capability_decisions', 'domain_decisions']) {
    ...
    for (const row of bundle[group]) {
      invariant(row.actual_reverse_engineering_executed === true, `${group} lacks actual reverse engineering`);
...
function markdownReport(report) {
  const lines = [
    '# Residual P5 route wave — exact-build deep recovery audit',
    '',
  ...
  lines.push('', '## Boundary', '',
    'The factory/constructor/vtable/deserializer chains are exact-build structural truth. The static MakeFunction surface and full PE function-range immediate scan do not recover packet-specific callback RTTI for these routes; the remaining receive owner lives in heap callback-tree state. No gameplay field role is inferred from adjacency, timing, payload shape, or protected object bytes.', '');
  return lines.join('\n');
}
