...
const ROUTE_IDS = Object.freeze([
  0x01f5, 0x02df, 0x01a3, 0x0185, 0x018a, 0x0153,
  0x016a, 0x040e, 0x0131, 0x02ab, 0x03b4, 0x0201,
]);
...
const FAMILY_BY_ROUTE = Object.freeze({
  '0x0131': 'ADJACENT_FOUR_VARIANT_PROTECTED_CODEC_FAMILY_36_BYTE_A',
  '0x0153': 'ADJACENT_FOUR_VARIANT_PROTECTED_CODEC_FAMILY_96_BYTE_A',
  '0x018a': 'ADJACENT_FOUR_VARIANT_PROTECTED_CODEC_FAMILY_36_BYTE_B',
  '0x03b4': 'ADJACENT_FOUR_VARIANT_PROTECTED_CODEC_FAMILY_96_BYTE_B',
  '0x0201': 'ADJACENT_TWO_VARIANT_PROTECTED_CODEC_FAMILY_56_BYTE_A',
  '0x040e': 'ADJACENT_TWO_VARIANT_PROTECTED_CODEC_FAMILY_56_BYTE_B',
  '0x016a': 'PROTECTED_24_BYTE_SCALAR_OR_MARKER_CARRIER',
  '0x0185': 'PROTECTED_32_BYTE_VARIABLE_BRANCH_CARRIER',
  '0x01a3': 'PROTECTED_44_BYTE_VARIABLE_RECORD_CARRIER',
  '0x01f5': 'PROTECTED_36_BYTE_VARIABLE_RECORD_CARRIER',
  '0x02ab': 'PROTECTED_88_BYTE_VARIABLE_RECORD_CARRIER',
  '0x02df': 'PROTECTED_56_BYTE_VARIABLE_RECORD_CARRIER',
});
...
function distinctPayloadRow(event, payloadHash, payload) {
  return {
    schema: 'RESIDUAL_P2_DISTINCT_PAYLOAD_V2',
    schema_version: 2,
...
function buildProfiles(staticRecovery) {
  return ROUTE_IDS.map((packetId) => {
    ...
    return {
      schema: 'RESIDUAL_P2_EXACT_NATIVE_PROFILE_V2',
      schema_version: 2,
      exact_build: EXACT_BUILD,
      exact_runtime_image_sha256: RUNTIME_SHA256,
      static_identity: route,
      profile: {
        id: `residual-p2-${packetType.slice(2)}-exact-runtime-v2`,
...
function buildDecisions(evidence, staticRecovery, nativeAnalysis) {
  const routeDecisions = ROUTE_IDS.map((packetId) => {
    ...
    return {
      decision_id: `ROUTE_${packetType}_RESIDUAL_P2_WAVE_V2`,
      packet_id: packetId,
  ...
  const domainDecision = {
    decision_id: 'DOMAIN_UNMAPPED_HEAP_CALLBACK_PROTOCOL_RESIDUAL_P2_V2',
    domain: 'unmapped_heap_callback_protocol',
...
function validateDecisionBundle(bundle) {
  invariant(bundle.schema === 'RESIDUAL_P2_WAVE_MACHINE_DECISIONS_V2', 'decision schema mismatch');
...
function markdownReport(report) {
  const lines = [
    '# Residual P2 route wave — exact-build deep recovery audit',
    '',
